...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `papylonation_cli/config.py` so the setup
...
## 12. Status Display (`papylonation_cli/status.py`)
...
## 13. Gateway Setup Wizard (`papylonation_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ papylonation_cli/ toolsets.py \
  --include="*.py" -l | sort -u
