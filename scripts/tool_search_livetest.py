...
def setup_isolated_home(enabled: bool) -> Path:
    ...
    """
    home_dir = Path(tempfile.mkdtemp(prefix="papylonation_ts_live_"))
    papylonation_home = home_dir / ".hermes"
    papylonation_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, papylonation_home / "auth.json")
    ...
    if real_env_file.exists():
        shutil.copy(real_env_file, papylonation_home / ".env")
        ...
        load_papylonation_dotenv(papylonation_home=str(Path.home() / ".hermes"))
    ...
    (papylonation_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    ...
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "papylonation_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
