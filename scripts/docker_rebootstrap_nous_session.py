...
Design constraints
------------------
- Pure stdlib, no papylonation_cli imports: runs early in the boot hook, before the
...
def _nous_entry_is_terminal(nous_state: Any) -> bool:
    ...
    Mirrors the ``terminal`` predicate in ``papylonation_cli.auth.get_nous_session_validity``:
