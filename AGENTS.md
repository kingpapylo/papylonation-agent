...
├── papylonation_state.py       # SessionDB — SQLite session store (FTS5 search)
├── papylonation_constants.py   # get_papylonation_home(), display_papylonation_home() — profile-aware paths
├── papylonation_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── papylonation_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
`gateway.log` when running the gateway. Profile-aware via `get_papylonation_home()`.
...
- **Skin engine** (`papylonation_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `HermesCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.hermes/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`papylonation_cli/commands.py`)
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `papylonation_cli/commands.py`:
```python
...
The dashboard embeds the real `hermes --tui` — **not** a rewrite.  See `papylonation_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `papylonation_cli/web_server.py`.
...
**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_papylonation_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `HERMES_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_papylonation_home()` for the base directory — never `Path.home() / ".hermes"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `papylonation_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `papylonation_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `hermes tools`, `hermes setup`, most CLI subcommands | `papylonation_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`papylonation_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
papylonation_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
...
Add to `_BUILTIN_SKINS` dict in `papylonation_cli/skin_engine.py`:
...
### General plugins (`papylonation_cli/plugins.py` + `plugins/<name>/`)
...
`post_setup(papylonation_home, config)` for setup-wizard integration.
...
(`run_agent.py`, `cli.py`, `gateway/run.py`, `papylonation_cli/main.py`, etc.).
...
- **CLI:** `papylonation_cli/curator.py` wires `hermes curator <verb>` where
...
- **CLI:** `papylonation_cli/kanban.py` wires `hermes kanban` with verbs
...
The core mechanism: `_apply_profile_override()` in `papylonation_cli/main.py` sets
`HERMES_HOME` before any module imports. All `get_papylonation_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_papylonation_home()` for all HERMES_HOME paths.** Import from `papylonation_constants`.
...
2. **Use `display_papylonation_home()` for user-facing messages.** Import from `papylonation_constants`.
   This returns `~/.hermes` for default or `~/.hermes/profiles/<name>` for profiles.
   ```python
   # GOOD
   from papylonation_constants import display_papylonation_home
   print(f"Config saved to {display_papylonation_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.hermes/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_papylonation_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_papylonation_home()`,
   not `Path.home() / ".hermes"`.

4. **Tests that mock `Path.home()` must also set `HERMES_HOME`** — since code now uses
   `get_papylonation_home()` (reads env var), not `Path.home() / ".hermes"`:
   ```python
...
6. **Profile operations are HOME-anchored, not HERMES_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".hermes" / "profiles"`, NOT `get_papylonation_home() / "profiles"`.
...
Use `get_papylonation_home()` from `papylonation_constants` for code paths. Use `display_papylonation_home()`
for user-facing print/log messages. Hardcoding `~/.hermes` breaks profiles — each profile
has its own `HERMES_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `papylonation_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `papylonation_cli/curses_ui.py` — see
`papylonation_cli/tools_config.py` for the canonical pattern.
...
The `_isolate_papylonation_home` autouse fixture in `tests/conftest.py` redirects `HERMES_HOME` to a temp dir. Never hardcode `~/.hermes/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_papylonation_home()` resolve within the temp dir.
Use the pattern from `tests/papylonation_cli/test_profiles.py`:
```python
@pytest.fixture
