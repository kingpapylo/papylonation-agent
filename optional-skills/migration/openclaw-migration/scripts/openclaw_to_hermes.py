...
class Migrator:
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
        ...
        for oc_key, papylonation_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and papylonation_key not in secret_additions:
                secret_additions[papylonation_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, papylonation_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and papylonation_key not in secret_additions:
                        secret_additions[papylonation_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        papylonation_config = load_yaml_file(destination)
        current_model = papylonation_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = papylonation_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                papylonation_config["model"] = {"default": model_str}
            dump_yaml_file(destination, papylonation_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        papylonation_config = load_yaml_file(destination)
        existing_tts = papylonation_config.get("tts", {})
        ...
        if self.execute:
            ...
            papylonation_config["tts"] = merged_tts
            dump_yaml_file(destination, papylonation_config)
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        papylonation_cfg_path = self.target_root / "config.yaml"
        papylonation_cfg = load_yaml_file(papylonation_cfg_path)
        existing_mcp = papylonation_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            papylonation_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                papylonation_srv["command"] = srv["command"]
                if srv.get("args"):
                    papylonation_srv["args"] = srv["args"]
                if srv.get("env"):
                    papylonation_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    papylonation_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                papylonation_srv["url"] = srv["url"]
                if srv.get("headers"):
                    papylonation_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    papylonation_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                papylonation_srv["enabled"] = False
            if srv.get("timeout"):
                papylonation_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                papylonation_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                papylonation_srv["tools"] = {}
                if tools_cfg.get("include"):
                    papylonation_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    papylonation_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                papylonation_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = papylonation_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(papylonation_cfg_path)
            papylonation_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(papylonation_cfg_path, papylonation_cfg)
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = papylonation_cfg.get("agent") or {}
        ...
        if compaction:
            compression = papylonation_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = papylonation_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            papylonation_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = papylonation_cfg.get("human_delay") or {}
        ...
        if changes:
            papylonation_cfg["agent"] = agent_cfg
            if self.execute:
                self.maybe_backup(papylonation_cfg_path)
                dump_yaml_file(papylonation_cfg_path, papylonation_cfg)
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        papylonation_cfg_path = self.target_root / "config.yaml"
        papylonation_cfg = load_yaml_file(papylonation_cfg_path)
        sr = papylonation_cfg.get("session_reset") or {}
        changes = False
        ...
        if changes:
            papylonation_cfg["session_reset"] = sr
            if self.execute:
                self.maybe_backup(papylonation_cfg_path)
                dump_yaml_file(papylonation_cfg_path, papylonation_cfg)
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        papylonation_cfg_path = self.target_root / "config.yaml"
        papylonation_cfg = load_yaml_file(papylonation_cfg_path)
        custom_providers = papylonation_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(papylonation_cfg_path)
            papylonation_cfg["custom_providers"] = custom_providers
            dump_yaml_file(papylonation_cfg_path, papylonation_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            papylonation_cfg_path = self.target_root / "config.yaml"
            papylonation_cfg = load_yaml_file(papylonation_cfg_path)
            discord_hermes = papylonation_cfg.get("discord") or {}
            changed = False
            ...
            if changed and self.execute:
                papylonation_cfg["discord"] = discord_hermes
                dump_yaml_file(papylonation_cfg_path, papylonation_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        papylonation_cfg_path = self.target_root / "config.yaml"
        papylonation_cfg = load_yaml_file(papylonation_cfg_path)
        browser_hermes = papylonation_cfg.get("browser") or {}
        changed = False
        ...
        if changed:
            papylonation_cfg["browser"] = browser_hermes
            if self.execute:
                self.maybe_backup(papylonation_cfg_path)
                dump_yaml_file(papylonation_cfg_path, papylonation_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            papylonation_mode = mode_map.get(mode, "manual")
            papylonation_cfg.setdefault("approvals", {})["mode"] = papylonation_mode
            if self.execute:
                self.maybe_backup(papylonation_cfg_path)
                dump_yaml_file(papylonation_cfg_path, papylonation_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{papylonation_mode}'")
