...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_papylonation_session_id: The live internal Hermes DB session id
            (``state.agent.session_id``).
        previous_papylonation_session_id: The internal id from before the most recent
    ...
    """
    try:
        row = db.get_session(current_papylonation_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_papylonation_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_papylonation_session_id}
    ...
    rotated = bool(
        ...
        and previous_papylonation_session_id != current_papylonation_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentHermesSessionId": current_papylonation_session_id,
    ...
    if previous_papylonation_session_id:
        provenance["previousHermesSessionId"] = previous_papylonation_session_id
    if rotated:
