...
DEFAULT_DB_PATH = get_papylonation_home() / "state.db"
...
# papylonation_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``papylonation_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see papylonation_cli/kanban_db.py) would
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_papylonation_fts_health_probe_{time.time_ns()}"
        try:
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``papylonation_state._set_last_init_error(None)`` explicitly.
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._papylonation_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._papylonation_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def get_messages_as_conversation(
        ...
        if repair_alternation and messages:
            # Lazy import: papylonation_state already depends on agent.* (see
