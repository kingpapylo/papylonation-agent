...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_papylonation_home() under the active profile.
...
#   - cli.py, papylonation_cli/*      -> inline on startup (no event loop)
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from papylonation_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from papylonation_cli.plugins import has_hook, invoke_hook
