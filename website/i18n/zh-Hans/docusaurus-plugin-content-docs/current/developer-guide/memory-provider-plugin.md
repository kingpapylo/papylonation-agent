...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs 始终包含：
          papylonation_home (str): 当前活跃的 HERMES_HOME 路径。用于存储数据。
        """
...
| `save_config(values, papylonation_home)` | 将非敏感配置写入原生位置 | **是**（除非仅使用环境变量） |
...
所有存储路径**必须**使用 `initialize()` 中的 `papylonation_home` kwarg，而不是硬编码的 `~/.hermes`：
