...
只有在你明确不想使用 Hermes managed install layout 时才使用这种方式（例如容器或 CI job 里的临时 clone）。如果这样安装，请确保运行的是这个 venv 里的 `hermes` entrypoint；运行系统 `python3 -m papylonation_cli.main` 可能会加载无关的系统 Python 包。
...
- **Profile 安全路径**：不得硬编码 `~/.hermes` — 代码路径使用 `papylonation_constants` 中的 `get_papylonation_home()`，面向用户的消息使用 `display_papylonation_home()`。完整规则参见 [AGENTS.md](https://github.com/NousResearch/hermes-agent/blob/main/AGENTS.md#profiles-multi-instance-support)。
