...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          papylonation_home (str): Active HERMES_HOME path. Use for storage.
        """
...
| `save_config(values, papylonation_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
```python
def save_config(self, values: dict, papylonation_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(papylonation_home) / "my-provider.json"
...
All storage paths **must** use the `papylonation_home` kwarg from `initialize()`, not hardcoded `~/.hermes`:

```python
# CORRECT — profile-scoped
from papylonation_constants import get_papylonation_home
data_dir = get_papylonation_home() / "my-provider"
