...
elif _papylonation_env="${HERMES_HOME:-$HOME/.hermes}/.env"; [ -f "$_papylonation_env" ] && grep -q "^GITHUB_TOKEN=" "$_papylonation_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_papylonation_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
