...
1. The agent writes a Python script using `from papylonation_tools import ...`
2. Hermes generates a `papylonation_tools.py` stub module with RPC functions
...
from papylonation_tools import web_search, web_extract
...
```python
from papylonation_tools import search_files, read_file
import json
...
```python
from papylonation_tools import search_files, read_file, patch
...
Hermes always writes the script and the auto-generated `papylonation_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
