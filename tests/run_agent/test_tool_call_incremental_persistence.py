...
def _make_agent():
    papylonation_home = Path(tempfile.mkdtemp(prefix="hermes-test-home-"))
    (papylonation_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._papylonation_home", papylonation_home),
