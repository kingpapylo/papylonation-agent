...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves papylonation_cli.kanban_db on its own
...
def main():
    home = tempfile.mkdtemp(prefix="papylonation_e2e_")
    ...
    # papylonation_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "hermes")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m papylonation_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/papylonation_e2e_*/worker_*.log
    import glob
