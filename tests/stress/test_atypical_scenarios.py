...
def scenario(name):
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"papylonation_atyp_{name}_")
            os.environ["HERMES_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("papylonation_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from papylonation_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "papylonation_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from papylonation_cli.kanban_db import resolve_workspace
            ...
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE papylonation_home: {resolved}")
...
@scenario("papylonation_home_with_spaces")
def _(home, kb):
...
@scenario("papylonation_home_via_symlink")
def _(home, kb):
    ...
    real = tempfile.mkdtemp(prefix="papylonation_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
...
def _idempotency_race_worker(papylonation_home: str, key: str, result_file: str,
                             barrier_path: str) -> None:
    """Subprocess body for the idempotency race test."""
    os.environ["HERMES_HOME"] = papylonation_home
    os.environ["HOME"] = papylonation_home
    sys.path.insert(0, str(WT))
...
def _(home, kb):
    ...
    try:
        from papylonation_cli import web_server as ws  # noqa
    except Exception:
        pass
