"""Tests for papylonation_bootstrap — Windows UTF-8 stdio shim.
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports papylonation_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported papylonation_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("papylonation_bootstrap", None)
    import papylonation_bootstrap  # noqa: WPS433
    return papylonation_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import papylonation_bootstrap as its
    ...
    # import papylonation_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "papylonation_cli/main.py",   # hermes CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import papylonation_bootstrap' and that
        ...
        ``papylonation_bootstrap`` as a top-level module) leaves hermes
        ...
        # at tests/test_papylonation_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when papylonation_bootstrap hasn't been
        ...
        assert first_import_name == "papylonation_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'papylonation_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import papylonation_bootstrap' line to be the first import."
        )
