...
class TestGetSubprocessHome:
    """Unit tests for papylonation_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(papylonation_constants, "is_container", lambda: False)
    ...
    def test_returns_none_when_papylonation_home_unset(self, monkeypatch):
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        papylonation_home = real_home / ".hermes" / "profiles" / "coder"
        profile_home = papylonation_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        from papylonation_constants import get_subprocess_home, get_real_home
    ...
    def test_real_home_falls_back_to_os_account_when_home_is_profile(self, tmp_path, monkeypatch):
        ...
        from papylonation_constants import get_real_home
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_papylonation_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_papylonation_home_override(profile)
        try:
            assert get_papylonation_home() == profile
            release.set()
        ...
        assert get_papylonation_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(papylonation_home / "home")
        assert result["HERMES_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_papylonation_home_unset(self, monkeypatch):
        monkeypatch.delenv("HERMES_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from papylonation_cli.profiles import _PROFILE_DIRS
...
class TestPythonProcessUnchanged:
    ...
    def test_path_home_unchanged_after_subprocess_home_resolved(
        ...
        assert sub_home in (None, str(papylonation_home / "home"), original_home)
