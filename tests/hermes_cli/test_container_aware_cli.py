...
def container_env(tmp_path, monkeypatch):
    ...
    container_mode = papylonation_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=hermes-agent\n"
        "exec_user=hermes\n"
        "papylonation_bin=/data/current-package/bin/hermes\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("papylonation_constants.is_container", return_value=False):
    ...
    assert info["papylonation_bin"] == "/data/current-package/bin/hermes"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("papylonation_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_papylonation_dev(container_env, monkeypatch):
...
def test_get_container_exec_info_not_skipped_when_papylonation_dev_zero(container_env, monkeypatch):
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        papylonation_home = Path(tmpdir) / ".hermes"
        papylonation_home.mkdir()
        (papylonation_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("papylonation_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_papylonation_home": lambda: papylonation_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        ...
        "exec_user=myuser\n"
        "papylonation_bin=/opt/hermes/bin/hermes\n"
    )
    ...
    assert info["papylonation_bin"] == "/opt/hermes/bin/hermes"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from papylonation_cli.main import _exec_in_container
