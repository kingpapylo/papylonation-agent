"""Tests for papylonation_cli.doctor."""
...
import papylonation_cli.doctor as doctor
import papylonation_cli.gateway as gateway_cli
from papylonation_cli import doctor as doctor_mod
from papylonation_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "HERMES_HOME", papylonation_home)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_papylonation_home(self, tmp_path, provider=""):
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_papylonation_home(tmp_path, provider)
...
def test_run_doctor_accepts_papylonation_provider_ids_that_catalog_aliases(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from papylonation_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("papylonation_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "papylonation_cli.doctor", raising=False)
        from papylonation_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir(parents=True)
        ...
        (papylonation_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "HERMES_HOME", papylonation_home)
        monkeypatch.setattr(doctor_mod, "get_papylonation_home", lambda: papylonation_home)
        ...
        return buf.getvalue(), papylonation_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, papylonation_home = self._run_config_section(
        ...
        assert "HERMES_MAX_ITERATIONS=90" in (papylonation_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (papylonation_home / ".env").read_text(encoding="utf-8")
