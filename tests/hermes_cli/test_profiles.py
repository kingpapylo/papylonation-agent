"""Comprehensive tests for papylonation_cli.profiles module.
...
from papylonation_cli.profiles import (
    ...
    import_profile,
    _get_profiles_root,
    _get_default_papylonation_home,
    seed_profile_skills,
...
def profile_env(tmp_path, monkeypatch):
    ...
    * HERMES_HOME  -> tmp_path/.hermes  (so get_papylonation_home() agrees)
...
class TestGetProfileDir:
    ...
    def test_default_returns_papylonation_home(self, profile_env):
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("papylonation_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("papylonation_cli.profiles._cleanup_gateway_service"), \
             patch("papylonation_cli.profiles.time.sleep"), \
             patch("papylonation_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_stops_profile_bound_backends_before_removal(self, profile_env):
        ...
        with patch("papylonation_cli.profiles._cleanup_gateway_service"), \
             patch("papylonation_cli.profiles._profile_bound_backend_pids", return_value=[4242]) as pids, \
    ...
    def test_rmtree_retries_transient_enotempty_then_succeeds(self, profile_env):
        ...
        with patch("papylonation_cli.profiles._cleanup_gateway_service"), \
             patch("papylonation_cli.profiles._profile_bound_backend_pids", return_value=[]), \
             patch("papylonation_cli.profiles.time.sleep"), \
             patch("papylonation_cli.profiles.shutil.rmtree", side_effect=flaky_rmtree):
            delete_profile("coder", yes=True)
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "papylonation_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "papylonation_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "papylonation_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "papylonation_cli.main", "--profile", "coder", "serve"]),
        ]
...
class TestGetActiveProfileName:
    ...
    def test_default_papylonation_home_returns_default(self, profile_env):
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("papylonation_cli.profiles.shutil.which", lambda name: "/opt/hermes/bin/hermes")
        from papylonation_cli.profiles import create_wrapper_script
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from papylonation_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from papylonation_cli.profiles import remove_wrapper_script
...
class TestWrapperScriptSecurity:
    ...
    def test_legit_alias_stays_inside_wrapper_dir(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from papylonation_cli.profiles import _get_wrapper_dir
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from papylonation_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from papylonation_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from papylonation_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("papylonation_cli.profiles.check_alias_collision", return_value="skip"):
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert cfg["hosts"]["papylonation_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["papylonation_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        assert "hermes.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["papylonation_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["papylonation_heimdall"]["workspace"] == "hermes"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                ...
                "papylonation_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        assert cfg["hosts"]["papylonation_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".papylonation_history",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.papylonation_history",
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_papylonation_home()."""
    ...
    def test_default_papylonation_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_papylonation_home()
    ...
    def test_default_papylonation_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_papylonation_home() returns HERMES_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(docker_home))
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        ...
        from papylonation_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from papylonation_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".hermes"
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_papylonation_home()
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_papylonation_home()
