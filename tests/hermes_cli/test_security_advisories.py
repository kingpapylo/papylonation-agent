"""Tests for papylonation_cli.security_advisories.
...
import papylonation_cli.security_advisories as adv
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full papylonation_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "papylonation_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "papylonation_cli.config.save_config",
