"""Tests for `_can_open_graphical_browser()` in papylonation_cli.auth.
...
from papylonation_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("papylonation_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.auth.sys.platform", "darwin")
