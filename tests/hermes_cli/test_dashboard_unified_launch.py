...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "papylonation_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        assert env.get("HERMES_HOME") == str(get_default_papylonation_root())
    ...
    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "papylonation_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
        ...
        # get_default_papylonation_root() strips the trailing profiles/<name>, so the
    ...
    def test_default_profile_launch_skips_routing(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "papylonation_cli.profiles.get_active_profile_name", lambda: "default"
        )
        listening_calls = []
        monkeypatch.setattr(
            main_mod, "_dashboard_listening",
    ...
    def test_dashboard_starts_mcp_discovery_for_ws_backend(self, main_mod, monkeypatch):
        ...
        calls = []
        monkeypatch.setattr(
            "papylonation_cli.mcp_startup.start_background_mcp_discovery",
