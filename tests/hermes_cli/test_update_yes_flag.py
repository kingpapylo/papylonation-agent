...
from papylonation_cli.main import cmd_update
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        if "status" in joined and "--porcelain" in joined:
            out = " M papylonation_cli/main.py\n" if dirty else ""
...
class TestUpdateYesConfigMigration:
    ...
    @patch("papylonation_cli.config.migrate_config")
    @patch("papylonation_cli.config.check_config_version", return_value=(1, 2))
    @patch("papylonation_cli.config.get_missing_config_fields", return_value=[])
    @patch("papylonation_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``papylonation_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``papylonation_cli.main`` first could leave
