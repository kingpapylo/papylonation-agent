"""Tests for papylonation_cli.service_manager — the abstract ServiceManager
...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    monkeypatch.setattr(
        "papylonation_cli.service_manager._s6_running", lambda: True,
    )
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from papylonation_cli.service_manager import _s6_running
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "papylonation_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "papylonation_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "papylonation_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "papylonation_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "papylonation_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "papylonation_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "papylonation_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "papylonation_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `papylonation_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import papylonation_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("papylonation_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "papylonation_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "papylonation_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "papylonation_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    monkeypatch.setattr(
        "papylonation_cli.service_manager.detect_service_manager", lambda: "s6",
    )
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from papylonation_cli.service_manager import _seed_supervise_skeleton
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    # in papylonation_cli/gateway.py::_gateway_command_inner.
...
def test_s6_register_staging_dir_is_dotfile_hidden_from_svscan(
    ...
    """
    import papylonation_cli.service_manager as sm
...
def test_s6_lifecycle_persists_named_profile_desired_state(
    ...
    profile_dir = papylonation_home / "profiles" / "coder"
...
def test_s6_lifecycle_persists_default_profile_desired_state(
    ...
    monkeypatch.setenv("HERMES_HOME", str(papylonation_home / "profiles" / "coder"))

    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.start("gateway-default")
    state = json.loads((papylonation_home / "gateway_state.json").read_text())
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from papylonation_cli.service_manager import S6CommandError
