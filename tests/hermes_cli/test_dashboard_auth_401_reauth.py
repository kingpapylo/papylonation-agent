...
Verifies the contract documented in Phase 6 v2 of the plan:
  ...
  - ``set_session_cookies(refresh_token="")`` does NOT emit the
    ``papylonation_session_rt`` cookie (contract V1: no RT to persist).
...
from papylonation_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from papylonation_cli.dashboard_auth.cookies import (
    SESSION_AT_COOKIE,
...
class TestTransparentRefreshOnAccessTokenEviction:
    ...
    the browser deletes ``papylonation_session_at`` the instant the token lapses,
    while ``papylonation_session_rt`` lives for 30 days. From that moment the
...
class TestAutoSsoRedirect:
    ...
    from papylonation_cli.dashboard_auth.cookies import SSO_ATTEMPT_COOKIE
...
class TestNextSameOriginValidation:
    ...
    def test_safe_next_validator_accepts_same_origin(self):
        from papylonation_cli.dashboard_auth.middleware import _safe_next_target
...
class TestValidatePostLoginTarget:
    ...
    def test_accepts_same_origin_paths(self):
        from papylonation_cli.dashboard_auth.routes import _validate_post_login_target
