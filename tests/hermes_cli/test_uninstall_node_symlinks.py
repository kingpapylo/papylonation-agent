"""Tests for papylonation_cli.uninstall.remove_node_symlinks.
...
def _make_papylonation_node(papylonation_home: Path) -> Path:
    """Create a fake $HERMES_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = papylonation_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_papylonation_node(fake_home):
    papylonation_home = fake_home / ".hermes"
    node_bin = _make_papylonation_node(papylonation_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(papylonation_home)
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    papylonation_home = fake_home / ".hermes"
    _make_papylonation_node(papylonation_home)
    local_bin = fake_home / ".local" / "bin"
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(papylonation_home) == []


def test_removes_dangling_symlink_into_papylonation_node(fake_home):
