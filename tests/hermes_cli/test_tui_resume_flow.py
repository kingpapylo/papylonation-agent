...
def main_mod(monkeypatch):
    import papylonation_cli.main as mod
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    assert "PapyloNation Agent v" in out
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_papylonation_home", lambda: tmp_path)
...
def test_oneshot_fails_closed_on_empty_final_response(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    import papylonation_cli.oneshot as oneshot_mod
...
def test_oneshot_exit_code_when_failed_without_response(monkeypatch):
    ...
    monkeypatch.setattr(
        "papylonation_cli.oneshot._run_agent",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from papylonation_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    from papylonation_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "papylonation_state", mod("papylonation_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "papylonation_cli.config",
        mod("papylonation_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "papylonation_cli.models",
        mod("papylonation_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "papylonation_cli.tools_config",
        mod("papylonation_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_papylonation_home
):
    captured = {}
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("papylonation_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_make_tui_argv_dev_prebuilds_papylonation_ink(monkeypatch, main_mod, tmp_path):
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "papylonation_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
