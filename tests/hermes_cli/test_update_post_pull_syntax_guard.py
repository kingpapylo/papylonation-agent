...
(e.g. orphan merge-conflict markers in ``papylonation_cli/config.py``), the CLI
...
markers in ``papylonation_cli/config.py``; users who ran ``hermes update`` in
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    assert papylonation_main._capture_head_sha(["git"], tmp_path) == "deadbeefcafe"
...
def test_capture_head_sha_returns_none_on_git_failure(monkeypatch, tmp_path):
    ...
    assert papylonation_main._capture_head_sha(["git"], tmp_path) is None
...
def _populate_critical_tree(root: Path, *, broken_file: str | None = None) -> None:
    ...
    for relpath in papylonation_main._UPDATE_CRITICAL_FILES:
        path = root / relpath
...
def test_validate_critical_files_syntax_ok_when_all_files_parse(tmp_path):
    ...
    ok, failing_path, error = papylonation_main._validate_critical_files_syntax(tmp_path)
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="papylonation_cli/config.py")

    ok, failing_path, error = papylonation_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("papylonation_cli/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="papylonation_cli/main.py")

    ok, failing_path, _ = papylonation_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("papylonation_cli/main.py")


def test_validate_critical_files_syntax_detects_break_in_web_server(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="papylonation_cli/web_server.py")

    ok, failing_path, _ = papylonation_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("papylonation_cli/web_server.py")
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except papylonation_constants.py
    for relpath in papylonation_main._UPDATE_CRITICAL_FILES:
        if relpath == "papylonation_constants.py":
            continue
...
def test_production_tree_passes_syntax_guard():
    ...
    ok, failing_path, error = papylonation_main._validate_critical_files_syntax(repo_root)
