...
from papylonation_cli.status import _effective_provider_label


def _label_with(config, env_base_url=""):
    with patch("papylonation_cli.status.resolve_requested_provider", return_value="auto"), \
         patch("papylonation_cli.status.resolve_provider", return_value="openrouter"), \
         patch("papylonation_cli.status.load_config", return_value=config), \
         patch("papylonation_cli.status.get_env_value",
...
def test_non_openrouter_provider_untouched():
    with patch("papylonation_cli.status.resolve_requested_provider", return_value="anthropic"), \
         patch("papylonation_cli.status.resolve_provider", return_value="anthropic"):
