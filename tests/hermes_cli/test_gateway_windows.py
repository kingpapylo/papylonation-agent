"""Tests for papylonation_cli.gateway_windows."""
...
import papylonation_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda papylonation_home: "")
    monkeypatch.setattr("papylonation_cli.config.get_papylonation_home", lambda: str(papylonation_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "papylonation_cli.main"]
    assert cwd == str(papylonation_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_papylonation_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr("papylonation_cli.config.get_papylonation_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("papylonation_cli.config.get_papylonation_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_papylonation_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    papylonation_home = tmp_path / "hermes-home"
    papylonation_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(papylonation_home.resolve()))}" in content
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "pythonw.exe" in content
    assert "papylonation_cli.main" in content
    assert "gateway run" in content
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("papylonation_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
