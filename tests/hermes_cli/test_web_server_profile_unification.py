...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_papylonation_home):
    ...
    default_home = get_papylonation_home()
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import papylonation_cli.mcp_config as mcp_config
        from papylonation_constants import get_papylonation_home
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        monkeypatch.setattr(
            web_server,
            "_spawn_papylonation_action",
            lambda subcommand, name: calls.append(list(subcommand)) or _FakeProc(),
        )
        monkeypatch.setattr(
            "papylonation_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_papylonation_home()))
            return None
        ...
        assert resp.json()["papylonation_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_papylonation_home(self, isolated_profiles, monkeypatch):
        import papylonation_cli.web_server as web_server

        monkeypatch.setattr(
            "papylonation_cli.main._make_tui_argv",
