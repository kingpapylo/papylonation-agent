...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from papylonation_cli import config as config_mod
    from papylonation_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_papylonation_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    try:
        with patch("papylonation_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
