"""Tests for papylonation_cli.mcp_catalog and papylonation_cli.mcp_picker.
...
@pytest.fixture(autouse=True)
def _isolate_papylonation_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp HERMES_HOME."""
    hh = tmp_path / "hermes-home"
    hh.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(hh))
    monkeypatch.setattr(
        "papylonation_cli.config.get_papylonation_home", lambda: hh
    )
    monkeypatch.setattr(
        "papylonation_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "papylonation_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_papylonation_home() lazily through papylonation_constants
    monkeypatch.setattr(
        "papylonation_constants.get_papylonation_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from papylonation_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from papylonation_cli.mcp_catalog import list_catalog
    ...
    def test_transport_env_parsed_and_written_to_server_config(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from papylonation_cli.mcp_catalog import _build_server_config
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from papylonation_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from papylonation_cli.config import load_config
        from papylonation_cli.mcp_catalog import CatalogError, install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from papylonation_cli.mcp_catalog import install_entry
        from papylonation_cli.config import get_env_value, load_config
    ...
    def test_install_required_env_missing_raises(self, catalog_dir, monkeypatch):
        ...
        from papylonation_cli import mcp_catalog
        from papylonation_cli.mcp_catalog import install_entry, CatalogError
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from papylonation_cli.mcp_catalog import install_entry, uninstall_entry
        from papylonation_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from papylonation_cli.mcp_catalog import uninstall_entry
...
class TestPicker:
    def test_show_catalog_empty(self, catalog_dir, capsys):
        from papylonation_cli.mcp_picker import show_catalog
    ...
    def test_install_by_name_unknown(self, catalog_dir, capsys):
        from papylonation_cli.mcp_picker import install_by_name
    ...
    def test_run_picker_non_tty_falls_back(self, catalog_dir, capsys, monkeypatch):
        ...
        from papylonation_cli.mcp_picker import run_picker
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from papylonation_cli.mcp_catalog import list_catalog, catalog_diagnostics
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from papylonation_cli import mcp_catalog
        from papylonation_cli.mcp_catalog import _do_git_install
    ...
    def test_branch_ref_uses_branch_clone(self, catalog_dir, monkeypatch):
        ...
        from papylonation_cli import mcp_catalog
        from papylonation_cli.mcp_catalog import _do_git_install, get_entry
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "papylonation_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        from papylonation_cli.mcp_catalog import _catalog_root, _parse_manifest
