...
def test_validator_flags_ssh_key_persistence_payload():
    ...
    warnings = validate_mcp_server_entry("h1781406356", _papylonation_0day_entry())
    assert warnings
...
def test_save_rejects_papylonation_0day_persistence_entry():
    from papylonation_cli.config import load_config
    from papylonation_cli.mcp_config import _save_mcp_server

    assert _save_mcp_server("h1781406356", _papylonation_0day_entry()) is False
...
def test_mcp_add_rejects_dangerous_entry_before_probe(monkeypatch, capsys):
    ...
    monkeypatch.setattr("papylonation_cli.mcp_config._probe_single_server", _probe_should_not_run)
...
def test_probe_rejects_dangerous_entry_before_connect(monkeypatch):
    from papylonation_cli.mcp_config import _probe_single_server
...
def test_runtime_loader_skips_dangerous_entry(monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.config.load_config", lambda: {"mcp_servers": servers})
...
def test_migration_disables_existing_dangerous_entry(tmp_path):
    ...
    from papylonation_cli.config import load_config, migrate_config
...
def test_dashboard_mcp_add_rejects_dangerous_entry():
    ...
    from papylonation_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
...
def test_profile_mcp_write_skips_dangerous_entry(tmp_path):
    from papylonation_cli.config import load_config
    from papylonation_cli.web_server import MCPServerCreate, _write_profile_mcp_servers
