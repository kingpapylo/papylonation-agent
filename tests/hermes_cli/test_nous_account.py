...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.auth.get_provider_auth_state", lambda provider: _state(token))
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("papylonation_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("papylonation_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
