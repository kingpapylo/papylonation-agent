...
def isolated_kanban_home(monkeypatch):
    ...
    for mod in list(sys.modules.keys()):
        if mod.startswith("papylonation_cli") or mod.startswith("papylonation_state") or mod == "papylonation_constants":
            del sys.modules[mod]
    yield test_home
...
def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "papylonation_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.config.load_config", lambda: fake_config)
...
def test_kanban_swarm_uses_existing_humanizer_skill():
    ...
    swarm_path = (
        ...
        / "papylonation_cli" / "kanban_swarm.py"
    )
