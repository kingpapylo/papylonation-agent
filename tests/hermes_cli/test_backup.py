...
def _make_papylonation_tree(root: Path) -> None:
    """Create a realistic ~/.hermes directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "papylonation_state.db"):
...
class TestShouldExclude:
    def test_excludes_papylonation_agent(self):
        from papylonation_cli.backup import _should_exclude
    ...
    def test_includes_nested_papylonation_agent_in_skills(self):
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        _make_papylonation_tree(papylonation_home)

        monkeypatch.setenv("HERMES_HOME", str(papylonation_home))
        # get_default_papylonation_root needs this
    ...
    def test_failed_sqlite_backup_never_raw_copies_live_wal_db(self, tmp_path, monkeypatch, capsys):
        ...
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        (papylonation_home / "config.yaml").write_text("model: test\n")
        db_path = papylonation_home / "state.db"
        ...
        import papylonation_cli.backup as backup_mod
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = papylonation_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, papylonation_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_papylonation_agent(self, tmp_path, monkeypatch):
    ...
    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        ...
        venv_pkg = papylonation_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = papylonation_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_papylonation_agent_in_skills(self, tmp_path, monkeypatch):
        ...
        nested = papylonation_home / "skills" / "autonomous-ai-agents" / "hermes-agent"
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        ...
        _symlink_file_or_skip(papylonation_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from papylonation_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only papylonation_state.db (old wrong name) is rejected."""
        from papylonation_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["papylonation_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        ...
        assert (papylonation_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (papylonation_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (papylonation_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (papylonation_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_papylonation_prefix(self, tmp_path, monkeypatch):
        ...
        assert (papylonation_home / "config.yaml").read_text() == "model: test\n"
        assert (papylonation_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_papylonation_zip(self, tmp_path, monkeypatch):
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (papylonation_home / "config.yaml").exists()
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (papylonation_home / "gateway_state.json").write_text(live_state)
        ...
        assert (papylonation_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (papylonation_home / "config.yaml").exists()
        assert not (papylonation_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        papylonation_home = tmp_path / ".hermes"
        (papylonation_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(papylonation_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (papylonation_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (papylonation_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            papylonation_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (papylonation_home / "gateway.pid").write_text("4242")
        (papylonation_home / "processes.json").write_text('{"live": true}')
        ...
        assert (papylonation_home / "gateway.pid").read_text() == "4242"
        assert (papylonation_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (papylonation_home / "cron.pid").exists()
        assert not (papylonation_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (papylonation_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("HERMES_HOME", str(papylonation_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (papylonation_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (papylonation_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (papylonation_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        _make_papylonation_tree(src_home)
        ...
        from papylonation_cli.backup import run_backup, run_import
...
class TestFormatSize:
    def test_bytes(self):
        from papylonation_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_hermes(self):
        ...
        import io
        from papylonation_cli.backup import _detect_prefix
...
class TestBackupEdgeCases:
    def test_nonexistent_papylonation_home(self, tmp_path, monkeypatch):
    ...
    def test_empty_papylonation_home(self, tmp_path, monkeypatch):
        """Backup handles empty hermes home (no files to back up)."""
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        # Only excluded dirs, no actual files
        (papylonation_home / "__pycache__").mkdir()
        (papylonation_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = papylonation_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = papylonation_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_hermes(self, tmp_path, monkeypatch):
        ...
        out_zip = papylonation_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        (papylonation_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("HERMES_HOME", str(papylonation_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        (papylonation_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("HERMES_HOME", str(papylonation_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = papylonation_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (papylonation_home / "config.yaml").exists()
        assert (papylonation_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (papylonation_home / "profiles" / "coder" / "config.yaml").exists()
        assert (papylonation_home / "profiles" / "researcher" / "config.yaml").exists()
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        def fake_import(name, *a, **kw):
            if name == "papylonation_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from papylonation_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def papylonation_home(self, tmp_path):
    ...
    def test_creates_snapshot(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)
        assert snap_id is not None
        snap_dir = papylonation_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", papylonation_home=papylonation_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)
        db_copy = papylonation_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)
        assert (papylonation_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)
        copied = papylonation_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)
        with open(papylonation_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(papylonation_home=empty) is None

    def test_max_file_size_skips_oversized_file(self, papylonation_home, capsys):
        ...
        snap_id = create_quick_snapshot(
            papylonation_home=papylonation_home, max_file_size=cap
        )
        assert snap_id is not None
        snap_dir = papylonation_home / "state-snapshots" / snap_id
    ...
    def test_max_file_size_none_copies_everything(self, papylonation_home):
        """Default (no cap) preserves manual /snapshot behavior."""
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home, max_file_size=None)
        assert (papylonation_home / "state-snapshots" / snap_id / "state.db").exists()

    def test_max_file_size_under_cap_copies(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(
            papylonation_home=papylonation_home, max_file_size=1 << 30
        )
        assert (papylonation_home / "state-snapshots" / snap_id / "state.db").exists()

    def test_list_snapshots(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", papylonation_home=papylonation_home)
        id2 = create_quick_snapshot(label="second", papylonation_home=papylonation_home)

        snaps = list_quick_snapshots(papylonation_home=papylonation_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", papylonation_home=papylonation_home)
        snaps = list_quick_snapshots(limit=3, papylonation_home=papylonation_home)
        assert len(snaps) == 3

    def test_restore_config(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)

        (papylonation_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (papylonation_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, papylonation_home=papylonation_home)
        assert result is True
        assert "openrouter" in (papylonation_home / "config.yaml").read_text()

    def test_restore_state_db(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)

        conn = sqlite3.connect(str(papylonation_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, papylonation_home=papylonation_home)
    ...
    def test_restore_nonexistent(self, papylonation_home):
        from papylonation_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", papylonation_home=papylonation_home) is False

    def test_auto_prune(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", papylonation_home=papylonation_home)
        snaps = list_quick_snapshots(limit=100, papylonation_home=papylonation_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", papylonation_home=papylonation_home)
        deleted = prune_quick_snapshots(keep=3, papylonation_home=papylonation_home)
        assert deleted == 7
        assert len(list_quick_snapshots(papylonation_home=papylonation_home)) == 3

    def test_snapshot_includes_pairing_directories(self, papylonation_home):
        ...
        (papylonation_home / "platforms" / "pairing").mkdir(parents=True)
        (papylonation_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (papylonation_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (papylonation_home / "pairing").mkdir()
        (papylonation_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (papylonation_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, papylonation_home):
        ...
        assert restore_quick_snapshot(snap_id, papylonation_home=papylonation_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, papylonation_home):
...
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, papylonation_home):
        ...
        for hostile in hostile_ids:
            assert restore_quick_snapshot(
                hostile, papylonation_home=papylonation_home
            ) is False, f"hostile snapshot_id was not rejected: {hostile!r}"

    def test_restore_rejects_manifest_rel_traversal(self, papylonation_home):
        ...
        escape_dst = papylonation_home.parent.parent / "outside.txt"
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from papylonation_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)
        copy = papylonation_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)
        copy = papylonation_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, papylonation_home):
        from papylonation_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(papylonation_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, papylonation_home):
        ...
        board_dir = papylonation_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)
        copy = (
            papylonation_home / "state-snapshots" / snap_id
    ...
    def test_additional_per_profile_dbs_round_trip(self, papylonation_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(papylonation_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, papylonation_home):
        ...
        board = papylonation_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(papylonation_home=papylonation_home)
        snap = papylonation_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, papylonation_home, monkeypatch):
        ...
        import papylonation_cli.backup as bk
        from papylonation_cli.backup import create_quick_snapshot
        ...
        copy = papylonation_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
...
class TestPreUpdateBackup:
    ...
    def papylonation_home(self, tmp_path):
        ...
        root.mkdir()
        _make_papylonation_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, papylonation_home):
        from papylonation_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(papylonation_home=papylonation_home)
        assert out is not None
        assert out.exists()
        assert out.parent == papylonation_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, papylonation_home):
    ...
    def test_does_not_recurse_into_prior_backups(self, papylonation_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from papylonation_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(papylonation_home=papylonation_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(papylonation_home=papylonation_home)
    ...
    def test_rotation_keeps_only_n(self, papylonation_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(papylonation_home=papylonation_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (papylonation_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, papylonation_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from papylonation_cli.backup import create_pre_update_backup

        (papylonation_home / "backups").mkdir(exist_ok=True)
        manual = papylonation_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(papylonation_home=papylonation_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from papylonation_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(papylonation_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, papylonation_home):
        ...
        """
        from papylonation_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(papylonation_home=papylonation_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, papylonation_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, papylonation_home):
        ...
        first = create_pre_update_backup(papylonation_home=papylonation_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(papylonation_home=papylonation_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(papylonation_home=papylonation_home, keep=0)
    ...
    def test_skips_symlinked_files(self, papylonation_home, tmp_path):
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def papylonation_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".hermes"
        root.mkdir()
        _make_papylonation_tree(root)
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for papylonation_cli.config + papylonation_constants so they pick up HERMES_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("papylonation_cli.config") or mod == "papylonation_constants":
                del __import__("sys").modules[mod]
        return root

    @staticmethod
    def _set_mode(papylonation_home, value):
        import yaml
        (papylonation_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("papylonation_cli.config"):
                del _sys.modules[mod]

    @staticmethod
    def _zips(papylonation_home):
        d = papylonation_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(papylonation_home):
        d = papylonation_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []

    def test_default_creates_quick_snapshot_only(self, papylonation_home, capsys):
        """With no config, the default mode is ``quick``: a state snapshot is
        created but NOT the full zip."""
        from papylonation_cli.main import _run_pre_update_backup
        ...
        assert self._snaps(papylonation_home)
        assert not self._zips(papylonation_home)

    def test_backup_flag_forces_full(self, papylonation_home, capsys):
        ...
        assert len(self._zips(papylonation_home)) == 1

    def test_no_backup_flag_skips_everything(self, papylonation_home, capsys):
        ...
        assert not self._snaps(papylonation_home)
        assert not self._zips(papylonation_home)

    def test_config_off_disables_everything_silently(self, papylonation_home, capsys):
        ...
        self._set_mode(papylonation_home, "off")
        from papylonation_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
    ...
    def test_legacy_false_maps_to_off(self, papylonation_home, capsys):
        """Legacy boolean ``false`` (the old zip opt-out) now means off."""
        self._set_mode(papylonation_home, False)
        from papylonation_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        assert snap_id is None
    ...
    def test_legacy_true_maps_to_full(self, papylonation_home, capsys):
        """Legacy boolean ``true`` (the old always-zip opt-in) means full."""
        self._set_mode(papylonation_home, True)
        from papylonation_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_config_full_mode(self, papylonation_home, capsys):
        self._set_mode(papylonation_home, "full")
        from papylonation_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_config_quick_mode(self, papylonation_home, capsys):
        self._set_mode(papylonation_home, "quick")
        from papylonation_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_unknown_mode_falls_back_to_quick(self, papylonation_home, capsys):
        self._set_mode(papylonation_home, "bogus-mode")
        from papylonation_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
    ...
    def test_no_backup_flag_overrides_full_config(self, papylonation_home, capsys):
    ...
    def test_backup_flag_overrides_off_config(self, papylonation_home, capsys):
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, papylonation_home):
        from papylonation_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(papylonation_home=papylonation_home)
        assert out is not None
        assert out.exists()
    ...
    def test_backup_uses_shared_exclusion_rules(self, papylonation_home):
    ...
    def test_restorable_with_papylonation_import(self, papylonation_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Hermes
        backup — `hermes import` should accept it."""
        from papylonation_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(papylonation_home=papylonation_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, papylonation_home):
        from papylonation_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(papylonation_home=papylonation_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(papylonation_home=papylonation_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, papylonation_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(papylonation_home=papylonation_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((papylonation_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_papylonation_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.hermes yet — nothing to back up."""
        from papylonation_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(papylonation_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, papylonation_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from papylonation_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(papylonation_home=papylonation_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(papylonation_home=papylonation_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    ...
    def _make_snapshot(self, papylonation_home: Path, label="pre-update"):
        from papylonation_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, papylonation_home=papylonation_home, keep=5)
    ...
    def test_restores_when_emptied_after_migration(self, tmp_path):
        ...
        papylonation_home = tmp_path / ".hermes"
        jobs_path = papylonation_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(papylonation_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, papylonation_home=papylonation_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = papylonation_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, papylonation_home=papylonation_home) is None
        assert restore_cron_jobs_if_emptied("", papylonation_home=papylonation_home) is None
...
class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, papylonation_home: Path) -> None:
        papylonation_home.mkdir(parents=True, exist_ok=True)
        (papylonation_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (papylonation_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (papylonation_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        papylonation_home = tmp_path / ".hermes"
        self._make_min_tree(papylonation_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        ...
        dst_home.mkdir()
        papylonation_home = dst_home / ".hermes"
        papylonation_home.mkdir()
        ...
        assert not (papylonation_home / "_external").exists()
