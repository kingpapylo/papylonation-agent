...
class TestSlackNativeSlashes:
    ...
    def test_papylonation_catchall_is_first(self):
    ...
    def test_telegram_parity(self):
        ...
        via_papylonation_norm = {_norm(n) for n in _SLACK_VIA_HERMES_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_papylonation_norm
        assert not missing, (
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from papylonation_cli import commands as commands_mod

        # `web` is enabled, `spotify` is disabled — enabling should only offer
        # the disabled ones.
        monkeypatch.setattr(
            "papylonation_cli.tools_config._get_platform_tools",
            lambda *_a, **_k: {"web", "file"},
        )
        monkeypatch.setattr("papylonation_cli.config.load_config", lambda: {})
        monkeypatch.setattr(
            "papylonation_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from papylonation_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch papylonation_cli.plugins.get_plugin_commands() to a fixed dict."""
        from papylonation_cli import plugins as _plugins_mod
