...
from papylonation_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateNpmLockfileCache:
    @staticmethod
    def _cache_file(papylonation_root, project_root):
        cache_key = hashlib.sha256(str(project_root).encode()).hexdigest()[:12]
        return papylonation_root / f".npm_lock_hash_{cache_key}"

    def test_npm_lockfile_changed_no_cache(self, tmp_path, monkeypatch):
        from papylonation_cli import main as hm
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        monkeypatch.setattr(papylonation_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            papylonation_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr("papylonation_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "papylonation_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "papylonation_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("papylonation_cli.config.check_config_version", return_value=(1, 2)), patch(
            "papylonation_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("papylonation_cli.main.sys") as mock_sys:
            ...
            from papylonation_cli.config import migrate_config
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "papylonation_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "papylonation_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "papylonation_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "papylonation_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "papylonation_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "papylonation_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "papylonation_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "papylonation_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("papylonation_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        with (
            patch("papylonation_cli.profiles.list_profiles", return_value=all_profiles),
            patch("papylonation_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("papylonation_cli.profiles.list_profiles", return_value=[default_p]),
            patch("papylonation_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateCheckBranchFlag:
    ...
    @patch("papylonation_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("papylonation_cli.config.detect_install_method", return_value="pip")
    @patch("papylonation_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
