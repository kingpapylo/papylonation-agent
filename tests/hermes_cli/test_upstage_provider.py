...
`papylonation_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageResolver:
    ...
    def test_solar_alias_normalizes_to_upstage(self):
        from papylonation_cli.providers import normalize_provider, resolve_provider_full
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `papylonation_cli/main.py` (cmd_model): read
