"""Tests for the Kanban DB layer (papylonation_cli.kanban_db)."""
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import papylonation_cli.kanban_db as _kb
...
def test_scratch_workspace_created_under_papylonation_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import papylonation_cli.profiles as profiles
...
class TestSharedBoardPaths:
    ...
    def _set_home(self, monkeypatch, tmp_path, papylonation_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_docker_custom_papylonation_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: HERMES_HOME points outside ~/.hermes.
        # `get_default_papylonation_root()` returns env_home directly when it
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # `get_default_papylonation_root()` walks up to /opt/hermes because
    ...
    def test_explicit_override_via_papylonation_kanban_home(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_papylonation_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # HERMES_KANBAN_DB pins the file path directly and beats both
        # HERMES_KANBAN_HOME and the `get_default_papylonation_root()` path.
    ...
    def test_papylonation_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see papylonation_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("papylonation_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="papylonation_state"):
            conn = kb.connect()
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_papylonation_argv()
#
...
def test_resolve_papylonation_argv_prefers_path_shim(monkeypatch):
    """When `hermes` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import papylonation_cli.kanban_db as kb

    monkeypatch.delenv("HERMES_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/hermes")
    argv = kb._resolve_papylonation_argv()
    assert argv == ["/usr/local/bin/hermes"]


def test_resolve_papylonation_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    ...
    assert kb._resolve_papylonation_argv() == [os.path.abspath(".\\hermes.exe")]


def test_resolve_papylonation_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    assert kb._resolve_papylonation_argv() == [sys.executable, "-m", "papylonation_cli.main"]


def test_resolve_papylonation_argv_honors_papylonation_bin_path_override(monkeypatch, tmp_path):
    ...
    assert kb._resolve_papylonation_argv() == [str(shim)]


def test_resolve_papylonation_argv_papylonation_bin_bare_name_uses_path(monkeypatch, tmp_path):
    ...
    assert kb._resolve_papylonation_argv() == [str(path_hermes)]


def test_resolve_papylonation_argv_papylonation_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
...
def test_resolve_papylonation_argv_papylonation_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
...
def test_resolve_papylonation_argv_papylonation_bin_unresolved_bare_name_falls_back(monkeypatch):
...
def test_resolve_papylonation_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m papylonation_cli.main`.
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_papylonation_argv()
    assert argv == [sys.executable, "-m", "papylonation_cli.main"]


def test_resolve_papylonation_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `papylonation_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m papylonation_cli.main --version`
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="papylonation_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from papylonation_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("papylonation_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from papylonation_cli.kanban_db import connect, write_txn
    import papylonation_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from papylonation_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from papylonation_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("papylonation_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("papylonation_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.kanban_db.os.name", "nt")
    with patch("papylonation_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("papylonation_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("papylonation_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "papylonation_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("papylonation_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "papylonation_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("papylonation_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("papylonation_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("papylonation_cli.kanban_db._record_worker_exit"):
            with patch("papylonation_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
