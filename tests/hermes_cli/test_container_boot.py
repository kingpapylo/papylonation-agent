"""Tests for papylonation_cli.container_boot — the cont-init.d-time
...
from papylonation_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "papylonation_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under papylonation_home/profiles/<name>/."""
    p = papylonation_home / "profiles" / name
...
def _seed_default_root(
    ...
    *,
    ...
    if state is not None:
        (papylonation_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (papylonation_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (papylonation_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        papylonation_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_reconcile_log_rotates_when_size_exceeded(
    ...
    from papylonation_cli import container_boot
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        papylonation_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_is_dashboard_container_true_for_dashboard_argv(
    ...
    from papylonation_cli.container_boot import _is_dashboard_container
