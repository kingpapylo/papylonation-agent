...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from papylonation_cli.auth_commands import auth_add_command
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_papylonation_oauth_login_pure",
        lambda: {
            "access_token": token,
            "refresh_token": "refresh-token",
    ...
    entry = next(item for item in entries if item["source"] == "manual:papylonation_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:papylonation_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "papylonation_cli.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "papylonation_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "papylonation_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from papylonation_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from papylonation_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "papylonation_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from papylonation_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from papylonation_cli.auth_commands import auth_reset_command
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((papylonation_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from papylonation_cli.auth import _reset_config_provider
    ...
    with patch("papylonation_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from papylonation_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "papylonation_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("papylonation_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("papylonation_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("papylonation_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "papylonation_cli.auth_commands.time.time",
...
def test_auth_remove_env_seeded_clears_env_var(tmp_path, monkeypatch):
    ...
    env_path = papylonation_home / ".env"
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (papylonation_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from papylonation_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((papylonation_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from papylonation_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from papylonation_cli.auth import unsuppress_credential_source
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((papylonation_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    ...
    (papylonation_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (papylonation_home / ".env").read_text().strip() == ""
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from papylonation_cli.auth import is_source_suppressed
    from papylonation_cli.auth_commands import auth_add_command
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import papylonation_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_qwen_suppression(tmp_path, monkeypatch):
    ...
    import papylonation_cli.auth as ha
...
def test_seed_from_singletons_respects_papylonation_pkce_suppression(tmp_path, monkeypatch):
    """anthropic papylonation_pkce must not re-seed from ~/.hermes/.anthropic_oauth.json when suppressed."""
    papylonation_home = tmp_path / "hermes"
    papylonation_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("HERMES_HOME", str(papylonation_home))

    import yaml
    (papylonation_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (papylonation_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["papylonation_pkce"]},
    }))

    # Stub the readers so only papylonation_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_papylonation_oauth_credentials", lambda: {
    ...
    # papylonation_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "papylonation_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (papylonation_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "papylonation_cli.copilot_auth.get_copilot_api_token",
