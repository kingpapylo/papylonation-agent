...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("papylonation_cli.config.get_managed_system", return_value=None), \
         patch("papylonation_cli.config.get_papylonation_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("papylonation_cli.config.get_managed_system", return_value="NixOS"), \
         patch("papylonation_cli.config.get_papylonation_home", return_value=tmp_path):
        from papylonation_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from papylonation_cli.config import recommended_update_command_for_method
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    ...
    with patch("papylonation_cli.config.get_managed_system", return_value=None), \
         patch("papylonation_cli.config.get_papylonation_home", return_value=home):
        from papylonation_cli.config import detect_install_method
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    with patch("papylonation_cli.config.get_managed_system", return_value=None), \
         patch("papylonation_cli.config.get_papylonation_home", return_value=home), \
         patch("papylonation_cli.config._running_in_container", return_value=False):
        from papylonation_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("papylonation_cli.config.get_managed_system", return_value=None), \
         patch("papylonation_cli.config.get_papylonation_home", return_value=home), \
         patch("papylonation_cli.config._running_in_container", return_value=True):
        from papylonation_cli.config import detect_install_method
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    ...
    code.mkdir()
    home.mkdir()
    with patch("papylonation_cli.config.get_papylonation_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    with patch("papylonation_cli.config.get_managed_system", return_value=None), \
         patch("papylonation_cli.config.get_papylonation_home", return_value=tmp_path), \
         patch("papylonation_constants.is_container", return_value=True):
        from papylonation_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from papylonation_cli import banner

    hh = tmp_path / ".hermes"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("papylonation_cli.config.get_papylonation_home", return_value=hh), \
         patch("papylonation_constants.get_papylonation_home", return_value=hh):
        buf = io.StringIO()
