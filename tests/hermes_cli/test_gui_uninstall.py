"""Tests for papylonation_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import papylonation_cli.gui_uninstall as gu


def _make_agent(papylonation_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = papylonation_home / "hermes-agent"
    (agent_root / "papylonation_cli").mkdir(parents=True)
    (agent_root / "papylonation_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(papylonation_home: Path) -> None:
    """Create the source-built GUI artifacts a `hermes desktop` run produces."""
    desktop = papylonation_home / "hermes-agent" / "apps" / "desktop"
    ...
    (papylonation_home / "hermes-agent" / "node_modules").mkdir(parents=True)
    (papylonation_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(papylonation_home: Path) -> None:
    (papylonation_home / "config.yaml").write_text("x: 1\n")
    (papylonation_home / ".env").write_text("KEY=secret\n")
    (papylonation_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    papylonation_home = tmp_path / ".hermes"
    papylonation_home.mkdir()
    assert gu.agent_is_installed(papylonation_home) is False
    _make_agent(papylonation_home)
    assert gu.agent_is_installed(papylonation_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    papylonation_home = tmp_path / ".hermes"
    (papylonation_home / "hermes-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(papylonation_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    papylonation_home = tmp_path / ".hermes"
    _make_gui_build(papylonation_home)
    artifacts = gu.source_built_gui_artifacts(papylonation_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(papylonation_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(papylonation_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    papylonation_home = tmp_path / ".hermes"
    agent_root = _make_agent(papylonation_home)
    _make_gui_build(papylonation_home)
    _make_user_data(papylonation_home)
    ...
    removed = gu.uninstall_gui(papylonation_home)
    ...
    assert not (papylonation_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "papylonation_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (papylonation_home / "config.yaml").exists()
    assert (papylonation_home / ".env").exists()
    assert (papylonation_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(papylonation_home)
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(papylonation_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(papylonation_home)
    ...
    assert summary["papylonation_home"] == str(papylonation_home)
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    """
    import papylonation_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_papylonation_home", lambda: papylonation_home)
    ...
    from papylonation_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (papylonation_home / "config.yaml").exists()
    assert papylonation_home.exists()
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    ...
    papylonation_home = tmp_path / ".hermes"
    (papylonation_home / "hermes-agent" / "papylonation_cli").mkdir(parents=True)
    (papylonation_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not papylonation_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m papylonation_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_papylonation_home", lambda: papylonation_home)
    ...
    assert not (desktop / "release").exists()
    assert not (papylonation_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "papylonation_cli").exists()
    assert (papylonation_home / "config.yaml").exists()
