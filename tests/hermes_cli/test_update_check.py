"""Tests for the update check mechanism in papylonation_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("papylonation_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "papylonation_cli" / "banner.py"
    ...
    with patch("papylonation_cli.banner.subprocess.run") as mock_run, \
         patch("papylonation_cli.banner.check_via_pypi", return_value=0) as mock_pypi:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    with patch("papylonation_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    with patch("papylonation_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    with patch("papylonation_cli.banner.subprocess.run") as mock_run:
        with patch("papylonation_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    with patch("papylonation_cli.config.detect_install_method", return_value="docker"), \
         patch("papylonation_cli.banner.subprocess.run") as mock_run, \
         patch("papylonation_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("papylonation_cli.config.detect_install_method", return_value="pip"), \
         patch("papylonation_cli.banner.subprocess.run") as mock_run, \
         patch("papylonation_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from papylonation_cli.main import _invalidate_update_cache
