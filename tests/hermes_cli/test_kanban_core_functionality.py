...
Complements tests/papylonation_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/papylonation_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/papylonation_cli/test_kanban_db.py.
    monkeypatch.setenv("HERMES_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_parse_duration_accepts_formats():
    from papylonation_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: papylonation_cli.main doesn't propagate sub-command exit codes
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    ...
    r = _sp.run(
        [_sys.executable, "-m", "papylonation_cli.main", "kanban",
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from papylonation_cli import kanban as kb_cli
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``papylonation_cli.kanban_db`` fresh and uses that single
