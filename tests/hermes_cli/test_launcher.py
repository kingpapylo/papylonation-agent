...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `papylonation_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("papylonation_cli.main")

    def fake_main():
        called.append("papylonation_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "papylonation_cli.main", fake_main_module)
    ...
    assert called == ["papylonation_cli.main"]
