...
""",
        ...
        from papylonation_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        (papylonation_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("HERMES_HOME", str(papylonation_home))
        from papylonation_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from papylonation_cli.plugins import PluginContext, PluginManifest, PluginManager
