...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_papylonation_env(self):
...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = papylonation_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._papylonation_home_path", return_value=papylonation_home):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        papylonation_home = tmp_path / ".hermes"
        hub = papylonation_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_papylonation_home(self, tmp_path):
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        papylonation_home = tmp_path / ".hermes"
        cache = papylonation_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
