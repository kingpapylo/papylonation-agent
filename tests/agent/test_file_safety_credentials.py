...
def fake_home(tmp_path, monkeypatch):
    """Point ``_papylonation_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "papylonation_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_papylonation_home_path", lambda: home)
    return home
...
def test_arbitrary_papylonation_home_file_not_blocked(fake_home):
...
def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "papylonation_home" / "auth.json"
...
def test_identically_named_papylonation_files_outside_home_not_blocked(
    fake_home, tmp_path
):
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(fs, "_papylonation_home_path", lambda: profile)
