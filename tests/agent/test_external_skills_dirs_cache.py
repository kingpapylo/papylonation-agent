...
@pytest.fixture
def papylonation_home_with_config(tmp_path, monkeypatch):
...
def test_returns_configured_external_dir(papylonation_home_with_config):
    _home, external, _cfg = papylonation_home_with_config
    result = get_external_skills_dirs()
    assert result == [external.resolve()]


def test_cache_reuses_result_without_reparsing(papylonation_home_with_config):
    """Subsequent calls hit the cache and skip YAML parsing entirely."""
    _home, _external, _cfg = papylonation_home_with_config
...
def test_cache_invalidates_on_mtime_change(papylonation_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = papylonation_home_with_config
...
def test_returned_list_is_a_copy(papylonation_home_with_config):
