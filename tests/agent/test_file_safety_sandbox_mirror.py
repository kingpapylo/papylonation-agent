...
class TestClassifySandboxMirrorTarget:
    ...
    def test_sandboxes_segment_without_home_papylonation_returns_none(self, tmp_path):
    ...
    def test_sandboxes_segment_with_home_but_no_papylonation_returns_none(self, tmp_path):
...
class TestSandboxMirrorIsOrthogonalToCrossProfile:
    ...
    def test_same_profile_mirror_still_flagged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_papylonation_root_path", lambda: tmp_path)
        monkeypatch.setattr(fs, "_papylonation_home_path", lambda: tmp_path / "profiles" / "group1")
