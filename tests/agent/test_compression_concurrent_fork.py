...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``papylonation_state`` module, whose ``SessionDB``
    ...
    namespace = {"__name__": "papylonation_state"}
    source = '''
class SessionDB:
...
_NominalSessionDBImpostor.__module__ = "papylonation_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``papylonation_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = papylonation_state.SessionDB
    monkeypatch.setattr(papylonation_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(papylonation_state, "SessionDB", real_session_db_type)
