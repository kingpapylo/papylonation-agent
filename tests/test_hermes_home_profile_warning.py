"""Tests for get_papylonation_home() profile-mode fallback warning.
...
profile is active, get_papylonation_home() should:
...
def fresh_constants(monkeypatch, tmp_path):
    """Import papylonation_constants fresh and reset the one-shot warn flag."""
    import importlib
    import papylonation_constants
    importlib.reload(papylonation_constants)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("HERMES_HOME", raising=False)
    return papylonation_constants
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        result = fresh_constants.get_papylonation_home()
    ...
    def test_default_active_profile_no_warning(
        ...
        papylonation_dir = tmp_path / ".hermes"
        papylonation_dir.mkdir()
        (papylonation_dir / "active_profile").write_text("default\n")
        result = fresh_constants.get_papylonation_home()
        assert result == tmp_path / ".hermes"
        assert "HERMES_HOME fallback" not in capsys.readouterr().err
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        papylonation_dir = tmp_path / ".hermes"
        papylonation_dir.mkdir()
        (papylonation_dir / "active_profile").write_text("coder\n")
        ...
        fresh_constants.get_papylonation_home()
        fresh_constants.get_papylonation_home()
        err2 = capsys.readouterr().err
        assert "HERMES_HOME fallback" not in err2

    def test_papylonation_home_set_suppresses_warning(
    ...
    def test_unreadable_active_profile_no_crash(
        ...
        (papylonation_dir / "active_profile").write_bytes(b"\xff\xfe\x00\x00")
    ...
    def test_empty_active_profile_no_warning(
        ...
        papylonation_dir = tmp_path / ".hermes"
        papylonation_dir.mkdir()
        (papylonation_dir / "active_profile").write_text("")
