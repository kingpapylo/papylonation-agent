...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "papylonation_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "papylonation_plugins" not in sys.modules:
        ns = types.ModuleType("papylonation_plugins")
        ns.__path__ = []
        sys.modules["papylonation_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "papylonation_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["papylonation_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_papylonation_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_papylonation_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_papylonation_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
...
class TestBundledDiscovery:
    def _write_enabled_config(self, papylonation_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from papylonation_cli import plugins as pmod
