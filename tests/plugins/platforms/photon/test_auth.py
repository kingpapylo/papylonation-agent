...
@pytest.fixture
def tmp_papylonation_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
def test_store_project_credentials_writes_env(tmp_papylonation_home: Path) -> None:
    photon_auth.store_project_credentials(
        spectrum_project_id="sp-789",
        project_secret="sek-ret",
        dashboard_project_id="dash-1",
    )
    env_text = (tmp_papylonation_home / ".env").read_text()
    assert "PHOTON_PROJECT_ID=sp-789" in env_text
    assert "PHOTON_PROJECT_SECRET=sek-ret" in env_text


def test_store_user_numbers_round_trip(tmp_papylonation_home: Path) -> None:
