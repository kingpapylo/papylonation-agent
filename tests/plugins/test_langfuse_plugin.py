...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_papylonation_config(self, monkeypatch):
        ...
        # Drop any cached import of papylonation_cli.config.
        sys.modules.pop("papylonation_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "papylonation_cli.config" not in sys.modules, (
            "langfuse plugin imported papylonation_cli.config — regression toward "
