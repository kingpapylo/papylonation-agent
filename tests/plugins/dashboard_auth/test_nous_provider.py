...
class TestConfigYamlSource:
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``papylonation_cli.config.load_config``
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "papylonation_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        nous_plugin.register(ctx)
...
class TestStartLogin:
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "papylonation_session_pkce" in result.cookie_payload
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["papylonation_session_pkce"] != b.cookie_payload[
            "papylonation_session_pkce"
        ]
