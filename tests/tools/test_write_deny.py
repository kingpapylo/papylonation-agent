...
class TestWriteDenyExactPaths:
    ...
    def test_papylonation_env(self):
        # ``.env`` under the active HERMES_HOME (profile-aware, not just
        # ``~/.hermes``) must be write-denied. The hermetic test conftest
        # points HERMES_HOME at a tempdir — resolve via get_papylonation_home()
        # to match the denylist.
        from papylonation_constants import get_papylonation_home
        path = str(get_papylonation_home() / ".env")
        assert _is_write_denied(path) is True

    def test_papylonation_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        active.
        """
        root = tmp_path / "papylonation_root"
        ...
        from papylonation_constants import get_papylonation_home, get_default_papylonation_root
        assert get_papylonation_home() == profile_home
        assert get_default_papylonation_root() == root
...
class TestWriteAllowed:
    ...
    def test_papylonation_control_files_requested_writable(self):
        from papylonation_constants import get_papylonation_home
