...
class TestConfigureWindowsStdio:
    """``papylonation_cli.stdio.configure_windows_stdio`` wiring.
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("papylonation_cli.stdio", None)
        # Fresh import now; tests import from papylonation_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import papylonation_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("papylonation_cli.stdio", None)

    def test_no_op_on_posix(self):
        from papylonation_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("papylonation_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "papylonation_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, papylonation_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "papylonation_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call papylonation_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """papylonation_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from papylonation_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from papylonation_cli._subprocess_compat import resolve_node_command
...
# ---------------------------------------------------------------------------
# papylonation_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "papylonation_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / papylonation_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "papylonation_cli/tools_config.py",
            "papylonation_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_papylonation_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_papylonation_home" in source
        assert 'cache_dir = get_papylonation_home() / "cache" / "terminal"' in source
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_papylonation_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "papylonation_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "papylonation_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        papylonation_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "papylonation_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from papylonation_cli._subprocess_compat import" in block
    ...
    def test_watcher_rewrites_console_python_to_windowless(self):
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "HERMES_HOME) so the windowless base pythonw resolves papylonation_cli."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import papylonation_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "papylonation_cli.main", "gateway", "run"]
    ...
    def test_windows_rewrites_to_pythonw_and_preserves_tail(self):
        ...
        # ``from papylonation_cli.gateway import PROJECT_ROOT`` resolves from
        ...
        import papylonation_cli.config  # noqa: F401
        import papylonation_cli.gateway  # noqa: F401

        argv = [
            "C:/venv/Scripts/python.exe",
            "-m",
            "papylonation_cli.main",
            "--profile",
            "work",
            "gateway",
            "run",
        ...
        # Mock get_papylonation_home too: the real one calls Path.resolve(), which
        ...
        with mock.patch.object(gw.sys, "platform", "win32"), mock.patch.object(
            ...
            "papylonation_cli.config.get_papylonation_home", return_value="C:/hermes"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
