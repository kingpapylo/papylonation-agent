...
def _restore_tool_modules():
    original_papylonation_home = os.environ.get("HERMES_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "papylonation_cli"
        or name.startswith("papylonation_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_papylonation_home is None:
            os.environ.pop("HERMES_HOME", None)
        else:
            os.environ["HERMES_HOME"] = original_papylonation_home
        _reset_modules(("tools", "papylonation_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    papylonation_cli = types.ModuleType("papylonation_cli")
    papylonation_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["papylonation_cli"] = papylonation_cli
    papylonation_home = tmp_path / "hermes-home"
    os.environ["HERMES_HOME"] = str(papylonation_home)
    sys.modules["papylonation_cli.config"] = types.SimpleNamespace(
        get_papylonation_home=lambda: papylonation_home,
    )
    ...
    return {
        "snapshot_store": papylonation_home / "modal_snapshots.json",
