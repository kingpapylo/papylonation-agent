...
def _chown_papylonation_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    log_path: Path,
    *,
    papylonation_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    papylonation_home = target if papylonation_home is None else papylonation_home
    script = (
        "set -eu\n"
        f'HERMES_HOME="{papylonation_home}"\n'
        f"{_chown_papylonation_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_papylonation_tree "{target}"\n'
    )
...
def test_stage2_uses_symlink_safe_helper_for_papylonation_home_trees(stage2_text: str) -> None:
    assert 'chown_papylonation_tree "$HERMES_HOME/$sub"' in stage2_text
    assert 'chown_papylonation_tree "$HERMES_HOME/profiles"' in stage2_text
    assert 'chown_papylonation_tree "$HERMES_HOME/cron"' in stage2_text
...
def test_stage2_skips_top_level_chown_for_symlinked_papylonation_home(
    stage2_text: str,
) -> None:
