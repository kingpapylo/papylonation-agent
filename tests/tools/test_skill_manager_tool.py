...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("papylonation_cli.config.load_config", return_value={"skills": {}}):
...
def _curator_pass(tmp_path, *, monkeypatch):
    ...
    (``get_papylonation_home()``) resolves into the same tree the skill manager
    searches, and flips ``is_background_review()`` → True so the consolidation
    guard fires.
    """
    papylonation_home = tmp_path / ".hermes"
    skills_root = papylonation_home / "skills"
