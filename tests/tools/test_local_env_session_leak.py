...
from tools.environments.local import _make_run_env, _sanitize_subprocess_env, papylonation_subprocess_env
...
# --------------------------------------------------------------------------- #
# Non-terminal spawn surface (papylonation_subprocess_env) — sibling path
# --------------------------------------------------------------------------- #

def test_papylonation_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    """papylonation_subprocess_env (browser/ACP/CLI/TUI-host spawns) must not leak a
    ...
    env = papylonation_subprocess_env()
...
def test_papylonation_subprocess_env_bound_contextvar_wins(monkeypatch):
...
def test_papylonation_subprocess_env_unengaged_preserves_fallback(monkeypatch):
