...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._papylonation_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(papylonation_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(papylonation_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_papylonation_bin(self):
        """After _INSTALL_FAILED, manual install in HERMES_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        papylonation_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(papylonation_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(papylonation_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._papylonation_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == papylonation_bin
            assert _tirith_mod._resolved_path == papylonation_bin
            mock_clear.assert_called_once()
...
class TestHermesHomeIsolation:
    def test_papylonation_bin_dir_respects_papylonation_home(self):
        """_papylonation_bin_dir must use HERMES_HOME, not hardcoded ~/.hermes."""
        from tools.tirith_security import _papylonation_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"HERMES_HOME": tmpdir}):
            result = _papylonation_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_papylonation_home(self):
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        papylonation_home = os.getenv("HERMES_HOME")
        assert papylonation_home is not None, "HERMES_HOME should be set by conftest"
        assert "papylonation_test" in papylonation_home, "Should point to test temp dir"

    def test_get_papylonation_home_fallback(self):
        """Without HERMES_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_papylonation_home
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._papylonation_bin_dir",
