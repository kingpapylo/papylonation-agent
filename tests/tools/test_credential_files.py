...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        (papylonation_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(papylonation_home / "token.json")
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        (papylonation_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        (papylonation_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        (papylonation_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        papylonation_home = tmp_path / ".hermes"
        skills_dir = papylonation_home / "skills"
    ...
    def test_custom_container_base(self, tmp_path):
        papylonation_home = tmp_path / ".hermes"
        (papylonation_home / "skills").mkdir(parents=True)
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file one level above papylonation_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside papylonation_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        ...
        papylonation_home.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(papylonation_home))
        (papylonation_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_papylonation_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of HERMES_HOME must be allowed."""
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        subdir = papylonation_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file outside papylonation_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside papylonation_home pointing outside
        symlink = papylonation_home / "evil_link.json"
        try:
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, papylonation_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(papylonation_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(papylonation_home, [str(sensitive)])
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        ...
        (papylonation_home / "oauth.json").write_text("{}")
        self._write_config(papylonation_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        (papylonation_home / "cache" / "documents").mkdir(parents=True)
        (papylonation_home / "cache" / "audio").mkdir(parents=True)
        (papylonation_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(papylonation_home))
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_papylonation_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        papylonation_home = tmp_path / ".hermes"
        papylonation_home.mkdir()
        # Use legacy dir name with content — get_papylonation_dir prefers
        # populated old over new.
        legacy_doc = papylonation_home / "document_cache"
        legacy_img = papylonation_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(papylonation_home / "document_cache") in host_paths
        assert str(papylonation_home / "image_cache") in host_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        papylonation_home = tmp_path / ".hermes"
        img_dir = papylonation_home / "cache" / "images"
    ...
    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        papylonation_home = tmp_path / ".hermes"
        (papylonation_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(papylonation_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(papylonation_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        papylonation_home = tmp_path / ".hermes"
        doc_dir = papylonation_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        papylonation_home = tmp_path / ".hermes"
        ss_dir = papylonation_home / "cache" / "screenshots"
