...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_papylonation_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("papylonation_constants.display_papylonation_home", return_value="~/.hermes/profiles/coder"):
            prompt = build_session_context_prompt(ctx)
...
class TestSessionStoreRewriteTranscript:
    ...
    def store(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(papylonation_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
        config = GatewayConfig()
...
class TestGatewayRoutingTable:
    ...
    def test_legacy_sessions_json_imported_when_db_table_empty(self, tmp_path):
        ...
        import papylonation_state
        db = papylonation_state.SessionDB()
