...
class TestResolutionOrder:
    ...
    def test_source_profile_wins_over_routing(self, mock_runner, discord_source):
        ...
        with patch("papylonation_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("papylonation_cli.profiles.get_profile_dir") as mock_get_dir:
                with patch("papylonation_cli.profiles.profile_exists", return_value=True):
    ...
    def test_default_fallback_when_no_active(self, mock_runner, discord_source):
        ...
        with patch("papylonation_cli.profiles.get_active_profile_name", return_value=None):
            with patch("papylonation_cli.profiles.get_profile_dir") as mock_get_dir:
    ...
    def test_nonexistent_profile_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("papylonation_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("papylonation_cli.profiles.get_profile_dir") as mock_get_dir:
                ...
                with patch("papylonation_cli.profiles.profile_exists", return_value=False):
                    with patch("papylonation_constants.get_papylonation_home", return_value=Path("/hermes")):
...
class TestExceptionHandling:
    ...
    def test_get_profile_dir_exception_logs_warning(self, mock_runner, discord_source, caplog):
        ...
        with patch("papylonation_cli.profiles.get_active_profile_name", return_value="active"):
            with patch("papylonation_cli.profiles.get_profile_dir", side_effect=ValueError("Invalid profile name")):
                with patch("papylonation_constants.get_papylonation_home", return_value=Path("/hermes")):
                    with caplog.at_level(logging.WARNING):
                        result = mock_runner._resolve_profile_home_for_source(discord_source)
    ...
    def test_exception_with_no_profile_name(self, mock_runner, discord_source, caplog):
        ...
        with patch("papylonation_cli.profiles.get_active_profile_name", return_value=None):
            with patch("papylonation_cli.profiles.get_profile_dir", side_effect=RuntimeError("Filesystem error")):
                with patch("papylonation_constants.get_papylonation_home", return_value=Path("/hermes")):
                    mock_runner._profile_name_for_source = MagicMock(return_value=None)
