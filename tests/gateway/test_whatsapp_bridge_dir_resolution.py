...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "papylonation_constants.get_papylonation_home", lambda: papylonation_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into HERMES_HOME.
    assert not (papylonation_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_papylonation_home(tmp_path, monkeypatch):
    ...
    expected = papylonation_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    ...
    papylonation_home = tmp_path / "papylonation_home"
    mirror = papylonation_home / "scripts" / "whatsapp-bridge"
