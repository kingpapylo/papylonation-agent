...
async def test_auto_registers_plugin_commands_for_discord(adapter):
    ...
    with patch(
        "papylonation_cli.plugins.get_plugin_commands",
        return_value={
            "metricas": {
...
async def test_slash_command_registration_stays_under_discord_limit(adapter):
    ...
    with patch("papylonation_cli.plugins.get_plugin_commands", return_value=many_plugins):
        adapter._register_slash_commands()
...
async def test_auto_create_thread_uses_message_content_as_name(adapter):
    ...
    assert thread._papylonation_auto_thread_initial_name == "Hello world, how are you?"
...
@pytest.mark.asyncio
async def test_auto_create_thread_falls_back_to_papylonation_when_only_mentions(adapter):
...
async def test_auto_thread_source_carries_initial_name_for_semantic_rename(adapter, monkeypatch):
    ...
    thread = SimpleNamespace(
        id=999,
        name="raw user prompt",
        _papylonation_auto_thread_initial_name="raw user prompt",
    )
    adapter._auto_create_thread = AsyncMock(return_value=thread)
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    config_path = papylonation_dir / "config.yaml"
    config_path.write_text(yaml.dump({
        "discord": {"auto_thread": True},
    }))

    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("HERMES_HOME", str(papylonation_dir))
...
def test_register_skill_command_is_flat_not_nested(adapter):
    ...
    with patch(
        "papylonation_cli.commands.discord_skill_commands_by_category",
