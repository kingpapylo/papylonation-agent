...
def isolated_home(tmp_path, monkeypatch):
    ...
    These tests deliberately re-import ``papylonation_cli`` / ``gateway`` so each
    ...
    _saved = {
        ...
        if k.startswith(("papylonation_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (papylonation_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("papylonation_cli", "gateway")):
                del sys.modules[mod]
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    write_cfg("model:\n  provider: openrouter\n")
    for mod in list(sys.modules.keys()):
        if mod.startswith("papylonation_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("papylonation_cli.runtime_provider")
