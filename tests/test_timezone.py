"""
Tests for timezone support (papylonation_time module + integration points).
...
import papylonation_time


def _reset_papylonation_time_cache():
    """Reset the papylonation_time module cache (replacement for removed reset_cache)."""
    papylonation_time._cached_tz = None
    papylonation_time._cached_tz_name = None
    papylonation_time._cache_resolved = False


# =========================================================================
# papylonation_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def setup_method(self):
        _reset_papylonation_time_cache()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="papylonation_time"):
            result = papylonation_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["HERMES_TIMEZONE"] = "UTC"
        _reset_papylonation_time_cache()
        r1 = papylonation_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["HERMES_TIMEZONE"] = "Asia/Kolkata"
        _reset_papylonation_time_cache()
        r2 = papylonation_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        ...
        tz = papylonation_time.get_timezone()
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=papylonation_tz) which shifted
    ...
    def test_ensure_aware_normalizes_aware_to_papylonation_tz(self):
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        # of the naive timestamp exceeds _papylonation_now's wall time — this would
