...
def test_main_setup_runs_model_configuration(monkeypatch):
    ...
    def fake_papylonation_main():
        calls["argv"] = sys.argv[:]

    monkeypatch.setattr("papylonation_cli.main.main", fake_papylonation_main)
...
def test_main_setup_offers_browser_install_when_tty(monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.main.main", lambda: None)
...
def test_main_setup_browser_calls_ensure_dependency(monkeypatch):
    ...
    monkeypatch.setattr("papylonation_cli.dep_ensure.ensure_dependency", fake_ensure)
