...
class TestSysPathOrdering:
    ...
    def test_papylonation_time_importable(self):
        """papylonation_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _papylonation_now
        assert callable(_papylonation_now)

    def test_papylonation_constants_importable(self):
        """papylonation_constants should be importable from cron context."""
        from papylonation_constants import get_papylonation_home
        assert callable(get_papylonation_home)
