...
def cron_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_BUNDLES_DIR", str(papylonation_home / "skill-bundles"))
    ...
    monkeypatch.setattr(_skills_tool, "HERMES_HOME", papylonation_home)
    ...
    return papylonation_home, _scheduler


def _plant_skill(papylonation_home: Path, name: str, body: str) -> None:
    """Drop a SKILL.md into ~/.hermes/skills/<name>/ bypassing skills_guard."""
    skill_dir = papylonation_home / "skills" / name
...
def _plant_bundle(papylonation_home: Path, name: str, skills: list[str], instruction: str = "") -> None:
    """Drop a bundle YAML into ~/.hermes/skill-bundles/ and refresh cache."""
    bundles_dir = papylonation_home / "skill-bundles"
...
class TestBuildJobPromptScansSkillContent:
    def test_clean_skill_builds_normally(self, cron_env):
        papylonation_home, scheduler = cron_env
        _plant_skill(papylonation_home, "news-digest", "Fetch the top 5 headlines and summarize.")
    ...
    def test_skill_with_invisible_unicode_sanitized_not_blocked(self, cron_env):
        ...
        _plant_skill(papylonation_home, "zwsp-skill", "clean looking\u200bskill content")
    ...
    def test_skill_bundle_in_job_skills_loads_referenced_skills(self, cron_env):
        papylonation_home, scheduler = cron_env
        _plant_skill(papylonation_home, "alpha-skill", "Alpha guidance for the cron task.")
        _plant_skill(papylonation_home, "beta-skill", "Beta guidance for the cron task.")
        _plant_bundle(
            papylonation_home,
            "article-pipeline",
    ...
    def test_bundle_name_shadows_skill_name_for_cron_jobs(self, cron_env):
        papylonation_home, scheduler = cron_env
        _plant_skill(papylonation_home, "article-pipeline", "Standalone skill should not win.")
        _plant_skill(papylonation_home, "bundle-member", "Bundle member should win.")
        _plant_bundle(papylonation_home, "article-pipeline", ["bundle-member"])
...
class TestScriptOutputNotStrictScanned:
    ...
    def test_command_shapes_in_context_from_output_not_blocked(self, cron_env, monkeypatch):
        ...
        output_root = papylonation_home / "cron" / "output"
