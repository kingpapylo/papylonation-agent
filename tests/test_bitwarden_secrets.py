...
def papylonation_home(tmp_path, monkeypatch):
    ...
    # Some modules cache get_papylonation_home; clear if needed.
    import papylonation_constants
    if hasattr(papylonation_constants, "_HERMES_HOME_CACHE"):
        papylonation_constants._HERMES_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(papylonation_home, monkeypatch):
...
def test_install_bws_happy_path(papylonation_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(papylonation_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(papylonation_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from papylonation_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under papylonation_home/cache/."""
    home = tmp_path / ".hermes"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
