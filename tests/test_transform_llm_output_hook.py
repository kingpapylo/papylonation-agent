...
from papylonation_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(papylonation_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <papylonation_home>/plugins/<name> and opt it in."""
    plugin_dir = papylonation_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    papylonation_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        papylonation_home, "capture_hook",
        register_body=(
...
def test_hook_exception_does_not_replace_response(tmp_path, monkeypatch):
    ...
    papylonation_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        papylonation_home, "raising_hook",
        register_body=(
            'def _boom(**kw):\n'
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "papylonation_empty"))
