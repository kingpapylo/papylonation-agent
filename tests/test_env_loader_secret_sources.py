"""Tests for the secret-source tracking in ``papylonation_cli.env_loader``.
...
from papylonation_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_papylonation_dotenv()`` is called at module-import time from several
    hot modules (cli.py, papylonation_cli/main.py, run_agent.py, ...).  The
    ...
    # cli.py, papylonation_cli/main.py, run_agent.py, trajectory_compressor.py,
