...
// papylonation_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - papylonation_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - papylonation_session_rt: the OAuth refresh token. Long-lived (24h rotating,
...
const AT_COOKIE_VARIANTS = ['__Host-papylonation_session_at', '__Secure-papylonation_session_at', 'papylonation_session_at']
const RT_COOKIE_VARIANTS = ['__Host-papylonation_session_rt', '__Secure-papylonation_session_rt', 'papylonation_session_rt']
