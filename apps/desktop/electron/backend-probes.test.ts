...
// have papylonation_cli" (since `node -c "import papylonation_cli"` will exit
...
test('canImportHermesCli returns false when interpreter cannot run -c', () => {
  // node IS an interpreter, but `node -c "import papylonation_cli"` is a
...
test('hermes runtime import probe checks config dependencies', () => {
  ...
  // (papylonation_cli/env_loader.py); a mid-update venv missing python-dotenv
  // passed the old probe and produced an unrecoverable boot loop.
  assert.match(probe, /\bimport dotenv\b/)
  assert.match(probe, /\bimport papylonation_cli\.config\b/)
})
