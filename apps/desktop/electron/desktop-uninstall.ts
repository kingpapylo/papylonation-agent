...
/**
 * Map an uninstall mode to the `python -m papylonation_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `papylonation_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'papylonation_cli.uninstall', '--mode', mode]
}
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import papylonation_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import papylonation_cli` resolves from source
