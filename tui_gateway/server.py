...
from papylonation_constants import (
    ...
    get_papylonation_home_override,
...
from tools.environments.local import papylonation_subprocess_env
...
load_papylonation_dotenv(
    papylonation_home=_papylonation_home, project_env=Path(__file__).parent.parent / ".env"
)
...
_CRASH_LOG = os.path.join(_papylonation_home, "logs", "tui_gateway_crash.log")
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        env = papylonation_subprocess_env(inherit_credentials=True)
        if profile_home:
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from papylonation_cli.active_sessions import transfer_active_session
...
def _max_live_sessions() -> int:
    try:
        from papylonation_cli.active_sessions import coerce_max_concurrent_sessions
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_papylonation_home).resolve():
        return None
...
def _profile_scoped(handler):
    ...
    sprites install under its ``pets/`` dir (both resolve via ``get_papylonation_home``).
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from papylonation_cli.runtime_provider import canonical_custom_identity
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from papylonation_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    overlay; it does not deep-merge ``papylonation_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _papylonation_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_papylonation_home_override()
        home = override if isinstance(override, str) and override else _papylonation_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    papylonation_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    path = _papylonation_home / "config.yaml"
...
def resolve_skin() -> dict:
    try:
        from papylonation_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    ...
    try:
        from papylonation_cli.models import get_preferred_silent_default_model
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from papylonation_cli.models import detect_static_provider_for_model
...
def _load_approval_mode() -> str:
    from papylonation_cli.config import DEFAULT_CONFIG, _deep_merge
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from papylonation_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(
    ...
    *,
    ...
    custom_provs = None
    try:
        from papylonation_cli.config import get_compatible_custom_providers, load_config
    ...
    if agent:
        try:
            from papylonation_cli.context_switch_guard import merge_preflight_compression_warning
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from papylonation_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from papylonation_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from papylonation_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from papylonation_cli.config import load_config as _load_full_cfg
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses papylonation_cli.mcp_startup; TUI stdio keeps
    # its existing tui_gateway.entry-owned thread.
    try:
        from papylonation_cli.mcp_startup import wait_for_mcp_discovery
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_papylonation_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_papylonation_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_papylonation_home_override(init_home_token)
            if sid in _sessions:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from papylonation_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from papylonation_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from papylonation_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from papylonation_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from papylonation_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_papylonation_home() / "cache" / "pet-gen"
...
# (agent/billing_view.py + papylonation_cli/nous_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from papylonation_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from papylonation_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from papylonation_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from papylonation_cli.auth import step_up_nous_billing_scope
...
def _(rid, params: dict) -> dict:
    ...
    lines = [
        ...
        "",
        f"Session ID: {key}",
        f"Path: {display_papylonation_home()}",
    ]
...
def _(rid, params: dict) -> dict:
    ...
    saved_dir = get_papylonation_home() / "sessions" / "saved"
    try:
    ...
    path = saved_dir / f"papylonation_conversation_{timestamp}.json"
...
def _spawn_trees_root():
    ...
    root = get_papylonation_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _(rid, params: dict) -> dict:
    from papylonation_cli.input_sanitize import sanitize_user_prompt_text
    ...
    def run():
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_papylonation_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from papylonation_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from papylonation_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: papylonation_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from papylonation_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _papylonation_home / "images"
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from papylonation_cli.model_switch import parse_model_flags
...
def _is_repo_junk(root: str) -> bool:
    ...
    papylonation_home = os.path.realpath(str(get_papylonation_home()))

    return real == home or real == papylonation_home or real.startswith(papylonation_home + os.sep)
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    papylonation_home = os.path.normcase(os.path.realpath(str(get_papylonation_home())))
    return real == home or real == papylonation_home
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from papylonation_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_papylonation_home), "display": display_papylonation_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from papylonation_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    ``papylonation_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from papylonation_cli.config import reload_env
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m papylonation_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "papylonation_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=min(int(params.get("timeout", 240)), 600),
            cwd=os.getcwd(),
            # cli.exec runs `python -m papylonation_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    try:
        from papylonation_cli.commands import resolve_command
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from papylonation_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    paste_dir = _papylonation_home / "pastes"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from papylonation_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from papylonation_cli.config import is_managed, save_env_value
        from papylonation_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from papylonation_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from papylonation_cli.config import remove_env_value
...
def _format_live_help_output() -> str:
    try:
        from papylonation_cli.commands import COMMANDS_BY_CATEGORY
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from papylonation_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from papylonation_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from papylonation_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from papylonation_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from papylonation_cli.browser_connect import launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        from papylonation_cli.plugins import get_plugin_manager
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_papylonation_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from papylonation_cli.skills_hub import do_install
        ...
        if action == "browse":
            from papylonation_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from papylonation_cli.skills_hub import inspect_skill
