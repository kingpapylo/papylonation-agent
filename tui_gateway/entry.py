...
# from shadowing Hermes's own top-level modules.  ``papylonation_bootstrap`` lives at
...
import papylonation_bootstrap

papylonation_bootstrap.harden_import_path()
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``papylonation_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is None or not thread.is_alive():
        return
    try:
        from papylonation_cli.mcp_startup import _resolve_discovery_timeout
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``papylonation_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
    ...
    if thread is not None and thread.is_alive():
        return True
    try:
        from papylonation_cli.mcp_startup import (
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``papylonation_cli.mcp_startup`` thread used by the
    ...
    try:
        from papylonation_cli.mcp_startup import join_mcp_discovery as _startup_join
