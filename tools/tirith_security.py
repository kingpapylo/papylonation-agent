...
def _get_papylonation_home() -> str:
...
def _failure_marker_path() -> str:
    ...
    return os.path.join(_get_papylonation_home(), ".tirith-install-failed")
...
def _papylonation_bin_dir() -> str:
    """Return $HERMES_HOME/bin, creating it if needed."""
    d = os.path.join(_get_papylonation_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    ...
    try:
        ...
        dest = os.path.join(_papylonation_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    papylonation_bin = os.path.join(_papylonation_bin_dir(), "tirith")
    if os.path.isfile(papylonation_bin) and os.access(papylonation_bin, os.X_OK):
        _resolved_path = papylonation_bin
        _install_failure_reason = ""
        _clear_install_failed()
