...
# Cache validation (mirrors papylonation_cli/profiles.py::_count_skills, d5eee133e):
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_papylonation_home()}/.env manually."
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_papylonation_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from papylonation_cli.plugins import _get_disabled_plugins, get_plugin_manager
...
def skill_view(
    ...
    try:
        ...
        if isinstance(metadata, dict):
            papylonation_meta = metadata.get("hermes", {}) or {}

        tags = _parse_tags(papylonation_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            papylonation_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
