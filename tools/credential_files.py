...
def _resolve_papylonation_home() -> Path:
...
def register_credential_file(
    ...
    *relative_path* is relative to ``HERMES_HOME`` (e.g. ``google_token.json``).
    ...
    """
    papylonation_home = _resolve_papylonation_home()
    ...
    host_path = papylonation_home / relative_path
    ...
    containment_error = validate_within_dir(host_path, papylonation_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if os.path.isabs(rel):
                        ...
                        continue
                    host_path = papylonation_home / rel
                    containment_error = validate_within_dir(host_path, papylonation_home)
                    if containment_error:
                        logger.warning(
...
# Each tuple is (new_subpath, old_name) matching papylonation_constants.get_papylonation_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_papylonation_dir()`` for backward compatibility with old directory layouts.
    """
    from papylonation_constants import get_papylonation_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_papylonation_dir(new_subpath, old_name)
        if host_dir.is_dir():
