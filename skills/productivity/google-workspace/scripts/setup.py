...
# Ensure sibling modules (_papylonation_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _papylonation_home import display_papylonation_home, get_papylonation_home
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_papylonation_home()}/google_token.json")
