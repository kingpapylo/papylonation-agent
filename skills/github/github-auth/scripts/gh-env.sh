...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _papylonation_env="${HERMES_HOME:-$HOME/.hermes}/.env"; [ -f "$_papylonation_env" ] && grep -q "^GITHUB_TOKEN=" "$_papylonation_env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_papylonation_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
