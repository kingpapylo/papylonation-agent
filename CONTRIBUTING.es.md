...
- Implementan el mismo ABC `MemoryProvider` (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown` y opcionalmente `post_setup(papylonation_home, config)` para integración con el asistente de configuración
...
├── papylonation_state.py           # Base de datos de sesiones SQLite con búsqueda de texto completo FTS5, títulos de sesión
...
│
├── papylonation_cli/               # Implementaciones de comandos CLI
...
- **Persistencia de sesión**: Todas las conversaciones se almacenan en SQLite (`papylonation_state.py`) con búsqueda de texto completo y títulos de sesión únicos.
...
Añade al dict `_BUILTIN_SKINS` en `papylonation_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
