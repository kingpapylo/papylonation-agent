...
_USER_NAMESPACE = "_papylonation_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_papylonation_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_papylonation_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    ...
    try:
        ...
        d = get_papylonation_home() / "plugins"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _papylonation_user_memory.<name>.cli, usually
