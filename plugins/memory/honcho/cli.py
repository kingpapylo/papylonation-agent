...
def _local_config_path() -> Path:
    ...
    """
    return get_papylonation_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    cfg: dict, papylonation_host: dict
    ...
    pin = False
    for val in (
        papylonation_host.get("pinUserPeer"),
        papylonation_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in papylonation_host:
        aliases_src = papylonation_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in papylonation_host:
        prefix_src = papylonation_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(papylonation_host: dict) -> None:
    ...
    for key in _IDENTITY_MAPPING_KEYS:
        papylonation_host.pop(key, None)
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom papylonation_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    papylonation_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        papylonation_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(papylonation_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = papylonation_host.get("userPeerAliases")
    prefix = papylonation_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(papylonation_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(papylonation_host)
    papylonation_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        papylonation_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        papylonation_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        current_host_key = papylonation_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            papylonation_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(papylonation_host)
        ...
        if use_oauth:
            ...
            try:
                ...
                return
            papylonation_host["apiKey"] = cred.access_token
            papylonation_host["oauth"] = cred.oauth_block()
            # Default the peer prompt to the name entered at consent.
            if cred.consent_peer_name:
                papylonation_host["peerName"] = cred.consent_peer_name
    ...
    # --- 3. Identity ---
    current_peer = papylonation_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        papylonation_host["peerName"] = new_peer

    current_ai = papylonation_host.get("aiPeer") or cfg.get("aiPeer", "hermes")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        papylonation_host["aiPeer"] = new_ai

    current_workspace = papylonation_host.get("workspace") or cfg.get("workspace", "hermes")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        papylonation_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, papylonation_host)
    ...
    if run_mapping:
        peer_target = papylonation_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(papylonation_host)
            papylonation_host["pinUserPeer"] = False
            if prior_aliases:
                papylonation_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                papylonation_host, current_prefix, prefix_from_root,
            ...
            if merged:
                papylonation_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                papylonation_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                papylonation_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = papylonation_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        papylonation_host["observationMode"] = new_obs
    else:
        papylonation_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(papylonation_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        papylonation_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        papylonation_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = papylonation_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        papylonation_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = papylonation_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        papylonation_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                papylonation_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(papylonation_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            papylonation_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        papylonation_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        papylonation_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        papylonation_host["dialecticReasoningLevel"] = new_reasoning
    else:
        papylonation_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = papylonation_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        papylonation_host["sessionStrategy"] = new_strat

    papylonation_host["enabled"] = True
    papylonation_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        papylonation_config = load_config()
        papylonation_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(papylonation_config)
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from papylonation_cli.memory_setup import cmd_setup_provider
