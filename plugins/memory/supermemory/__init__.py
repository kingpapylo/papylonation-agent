...
def _load_supermemory_config(papylonation_home: str) -> dict:
    config = _default_config()
    config_path = Path(papylonation_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, papylonation_home: str) -> None:
    config_path = Path(papylonation_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
def _resolve_container_tag_for_setup(papylonation_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(papylonation_home)
...
def _probe_supermemory_connection(api_key: str, papylonation_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(papylonation_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(papylonation_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, papylonation_home):
        ...
        _save_supermemory_config(sanitized, papylonation_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, papylonation_home)
    ...
    def post_setup(self, papylonation_home: str, config: dict) -> None:
        ...
        from papylonation_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(papylonation_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from papylonation_constants import get_papylonation_home
        self._papylonation_home = kwargs.get("papylonation_home") or str(get_papylonation_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._papylonation_home)
