...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, papylonation_home):
        ...
        config_path = Path(papylonation_home) / "config.yaml"
        try:
            import yaml
            existing = {}
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_papylonation_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from papylonation_constants import get_papylonation_home
        _papylonation_home = str(get_papylonation_home())
        _default_db = _papylonation_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $HERMES_HOME in user-supplied paths so config values like
        # "$HERMES_HOME/memory_store.db" or "~/.hermes/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$HERMES_HOME", _papylonation_home)
            db_path = db_path.replace("${HERMES_HOME}", _papylonation_home)
