...
try:
    from papylonation_constants import display_papylonation_home, get_papylonation_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where papylonation_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _papylonation_home.py shim).
...
def _legacy_token_path() -> Path:
    return _papylonation_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _papylonation_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _papylonation_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _papylonation_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _papylonation_home() / "google_chat_user_client_secret.json"
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_papylonation_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_papylonation_home()}/google_chat_user_token.json"
    )
