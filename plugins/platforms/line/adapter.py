...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        try:
            ...
            papylonation_home = Path(get_papylonation_home()).resolve()
        except Exception:
            papylonation_home = Path.home().joinpath(".hermes").resolve()
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.hermes/.env`` via ``papylonation_cli.config``.
    """
    print()
    ...
    try:
        from papylonation_cli.config import get_env_var, set_env_var
    except ImportError:
        print("papylonation_cli.config not available; set LINE_* vars manually in ~/.hermes/.env")
        return
