...
"state": …}`` and the route serializes those into the ``papylonation_session_pkce``
cookie.
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"papylonation_session_pkce\"]``
        ...
        cookie_payload = {
            "papylonation_session_pkce": f"state={state};verifier={code_verifier}",
        }
...
def register(ctx) -> None:
    ...
    engagement layer (``papylonation_cli.web_server.should_require_auth`` +
