...
Three cookies in play:
  - papylonation_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - papylonation_session_rt:   the OAuth refresh token
  ...
  - papylonation_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-papylonation_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "papylonation_session_at"
SESSION_RT_COOKIE = "papylonation_session_rt"
...
SESSION_PROVIDER_COOKIE = "papylonation_session_provider"
PKCE_COOKIE = "papylonation_session_pkce"
...
SSO_ATTEMPT_COOKIE = "papylonation_sso_attempt"
