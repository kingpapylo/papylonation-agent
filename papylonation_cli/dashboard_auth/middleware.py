...
from papylonation_cli.dashboard_auth import list_session_providers
...
from papylonation_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    """
    from papylonation_cli.dashboard_auth.prefix import prefix_from_request
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from papylonation_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
async def gated_auth_middleware(
    ...
    # sends only ``papylonation_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from papylonation_cli.dashboard_auth.cookies import clear_session_cookies
        from papylonation_cli.dashboard_auth.prefix import prefix_from_request
