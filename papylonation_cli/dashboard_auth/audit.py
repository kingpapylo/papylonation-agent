...
from ``papylonation_constants`` or other papylonation_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    ...
    Mirrors ``papylonation_constants.get_papylonation_home`` semantics: env var wins,
    else ``~/.hermes``. A local copy avoids an import cycle with the
    middleware which lives below ``papylonation_cli``.
    """
