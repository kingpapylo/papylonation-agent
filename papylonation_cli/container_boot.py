...
def reconcile_profile_gateways(
    *,
    ...
    not under ``profiles/``). The dispatcher in ``papylonation_cli.gateway``
    ...
    Args:
        papylonation_home: The container's HERMES_HOME (typically /opt/data).
            Profiles live under ``<papylonation_home>/profiles/<name>/``;
            the default profile lives at ``<papylonation_home>`` itself.
    ...
    default_prior_state = legacy_default_state or _read_desired_state(papylonation_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(papylonation_home)
    ...
    profiles_root = papylonation_home / "profiles"
    ...
    if not dry_run:
        _write_reconcile_log(papylonation_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = papylonation_home / "gateway_state.json"
...
def _write_reconcile_log(
    papylonation_home: Path, actions: list[ReconcileAction],
) -> None:
    ...
    """
    import time
    log_dir = papylonation_home / "logs"
...
def main() -> int:
    ...
    actions = reconcile_profile_gateways(
        papylonation_home=papylonation_home, scandir=scandir,
    )
    for a in actions:
        print(
