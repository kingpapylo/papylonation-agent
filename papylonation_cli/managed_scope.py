...
This is DISTINCT from ``papylonation_cli.config.is_managed()`` / ``HERMES_MANAGED``,
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through papylonation_cli.config.load_config) to honor managed
    scope. Mirrors papylonation_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from papylonation_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
