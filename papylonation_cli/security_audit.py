...
def _discover_plugins(papylonation_home: Path) -> list[Component]:
...
def run_audit(
    *,
    ...
    home = papylonation_home or Path(get_papylonation_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, papylonation_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(papylonation_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, papylonation_home=home
    )
    if total == 0:
