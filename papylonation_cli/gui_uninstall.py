...
def _agent_root(papylonation_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return papylonation_home / "hermes-agent"
...
def source_built_gui_artifacts(papylonation_home: Path) -> "list[Path]":
    ...
    """
    agent_root = _agent_root(papylonation_home)
    ...
    return [
        ...
        papylonation_home / "desktop-build-stamp.json",
    ]
...
def agent_is_installed(papylonation_home: Path) -> bool:
    ...
    if (agent_root / "papylonation_cli").is_dir():
        return True
...
def gui_is_installed(papylonation_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(papylonation_home):
        if p.exists():
            return True
...
def gui_install_summary(papylonation_home: "Path | None" = None) -> dict:
...
def uninstall_gui(papylonation_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``hermes-agent/papylonation_cli`` (agent source), ``venv/``, or any
