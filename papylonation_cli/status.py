...
from papylonation_cli.auth import AuthError, resolve_provider
from papylonation_cli.colors import Colors, color
from papylonation_cli.config import get_env_path, get_env_value, get_papylonation_home, load_config
from papylonation_cli.models import provider_label
...
from papylonation_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    ...
    if _effective_provider_label() == "LM Studio":
        from papylonation_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from papylonation_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_papylonation_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_papylonation_home() / "sessions" / "sessions.json"
