...
``papylonation_cli/dump.py`` and ``papylonation_cli/banner.py``).  That doesn't work inside
...
arg into ``<project_root>/.papylonation_build_sha``.  This module is the single
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".papylonation_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.papylonation_build_sha`` if present.  The file is
