...
# display_papylonation_home is imported lazily at call sites to avoid ImportError
# when papylonation_constants is cached from a pre-update version during `hermes update`.
...
from papylonation_cli.colors import Colors, color
...
def _scan_gateway_pids(
    ...
    current_home = str(get_papylonation_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"papylonation_home={current_home_lc}" in command_lc
            )
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "papylonation_home=" in command_lc
            and f"papylonation_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "papylonation_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m papylonation_cli.main gateway
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    from papylonation_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from papylonation_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # base interpreter needs to import papylonation_cli.  Empty on POSIX, where
...
def _papylonation_home_from_systemd_unit_file(system: bool = False) -> str | None:
...
def _sync_papylonation_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``HERMES_HOME``.

    Under ``sudo``, ``HERMES_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_papylonation_home` falls back to ``/root/.hermes`` — the wrong
    ...
    unit_home = (_papylonation_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from papylonation_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from papylonation_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (papylonation_cli/gateway.py restart
...
from papylonation_constants import is_container, is_termux, is_wsl
...
def _profile_suffix() -> str:
    ...
    home = get_papylonation_home().resolve()
    default = get_default_papylonation_root().resolve()
    if home == default:
        return ""
...
def _profile_arg(papylonation_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        papylonation_home: Optional explicit HERMES_HOME path. Defaults to the current
            ``get_papylonation_home()`` value. Should be passed when generating a
        ...
        default_root: Optional Hermes root to compare against. Used when
            ...
            process, where ``Path.home()`` and ``get_default_papylonation_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from papylonation_constants import get_default_papylonation_root

    home = Path(papylonation_home or str(get_papylonation_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_papylonation_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(papylonation_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".hermes"
    try:
        Path(papylonation_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(papylonation_home, default_root=target_root)
    except ValueError:
        return _profile_arg(papylonation_home)
...
def _find_legacy_papylonation_units() -> list[tuple[str, Path, bool]]:
...
def has_legacy_papylonation_units() -> bool:
    """Return True when any legacy Hermes gateway unit files exist."""
    return bool(_find_legacy_papylonation_units())
...
def print_legacy_unit_warning() -> None:
    ...
    """
    legacy = _find_legacy_papylonation_units()
    if not legacy:
        return
...
def remove_legacy_papylonation_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Hermes gateway unit files.

    Iterates over whatever ``_find_legacy_papylonation_units()`` returns — which is
...
def _papylonation_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HERMES_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_papylonation_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_papylonation_home().resolve()
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    papylonation_node = papylonation_home / "node" / "bin"
    if _is_dir(papylonation_node):
        candidates.append(str(papylonation_node))
    papylonation_nm = papylonation_home / "node_modules" / ".bin"
    if _is_dir(papylonation_nm):
        candidates.append(str(papylonation_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m papylonation_cli.main``, so module
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        papylonation_home = _papylonation_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(papylonation_home, home_dir)
        ...
        working_dir = str(papylonation_home) if papylonation_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m papylonation_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="HERMES_HOME={papylonation_home}"
Restart=always
RestartSec=5
...
"""
    ...
    papylonation_home = str(get_papylonation_home().resolve())
    profile_arg = _profile_arg(papylonation_home)
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    _sync_papylonation_home_from_systemd_unit(system=system)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # generated unit bakes that ``/tmp/pytest-of-.../papylonation_test`` path into
    ...
    if not system and (
        ...
        or '/papylonation_test"' in new_unit
        or "/papylonation_test/" in new_unit
    ):
        return False
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_papylonation_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_papylonation_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m papylonation_cli.main [--profile X] gateway run --replace` argv.
    ...
    """
    cmd = [get_python_path(), "-m", "papylonation_cli.main"]
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>papylonation_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>HERMES_HOME</key>
        <string>{papylonation_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        reload_log_path = get_papylonation_home() / "logs" / "launchd-reload.log"
        try:
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_papylonation_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        from papylonation_constants import get_default_papylonation_root
        default_root = get_default_papylonation_root()
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_papylonation_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from papylonation_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_papylonation_home())))
    except KeyboardInterrupt:
        print()
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``papylonation_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from papylonation_cli import setup as _s
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`papylonation_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from papylonation_cli.service_manager import (
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            from papylonation_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_papylonation_units(interactive=not yes, dry_run=dry_run)
