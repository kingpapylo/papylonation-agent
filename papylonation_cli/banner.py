...
from papylonation_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def check_for_updates() -> Optional[int]:
    ...
    cache_file = papylonation_home / ".update_check"
    ...
    try:
        from papylonation_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        if not (repo_dir / ".git").exists():
            repo_dir = papylonation_home / "hermes-agent"
        if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``papylonation_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from papylonation_cli.build_info import get_build_sha
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]PapyloNation[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]PapyloNation[/]")
    ...
    try:
        from papylonation_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from papylonation_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
