...
the existing module-level functions in papylonation_cli.gateway and
papylonation_cli.gateway_windows directly. This protocol is a thin facade
...
# in ``papylonation_cli.gateway`` (systemd/launchd) and ``papylonation_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in papylonation_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from papylonation_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from papylonation_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from papylonation_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from papylonation_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in papylonation_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from papylonation_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from papylonation_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from papylonation_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from papylonation_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``papylonation_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    papylonation_home = Path(os.environ.get("HERMES_HOME", "/opt/data"))
    if papylonation_home.parent.name == "profiles":
        root = papylonation_home.parent.parent
    else:
        root = papylonation_home
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        ``papylonation_cli.gateway`` maps that empty string to the
        ...
        allocator in ``papylonation_cli.profiles._allocate_gateway_port``).
