...
from papylonation_cli.config import get_papylonation_home, get_config_path, load_config, save_config
...
def _cmd_migrate(args):
    ...
    print_info(f"Target:      {papylonation_home}")
    ...
    # Delegates to papylonation_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from papylonation_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(papylonation_home=papylonation_home)
            if backup_archive:
