...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "papylonation_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".papylonation_history",
    # User data
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            papylonation_requires=str(data.get("papylonation_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.papylonation_requires:
            out["papylonation_requires"] = self.papylonation_requires
        if self.author:
...
def check_papylonation_requires(spec: str, current_version: str) -> None:
...
def plan_install(
    ...
    from papylonation_cli import __version__ as papylonation_version
    ...
    check_papylonation_requires(manifest.papylonation_requires, papylonation_version)
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="papylonation_dist_install_") as tmp:
...
def update_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="papylonation_dist_update_") as tmp:
        plan = plan_install(
