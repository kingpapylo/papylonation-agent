...
from papylonation_cli.config import get_papylonation_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.papylonation_build_sha`` by the Dockerfile's
    ``HERMES_GIT_SHA`` build-arg (see ``papylonation_cli/build_info.py``).
...
def _gateway_status() -> str:
    ...
    try:
        from papylonation_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(papylonation_home: Path) -> int:
...
def _cron_summary(papylonation_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = papylonation_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    load_papylonation_dotenv(
        papylonation_home=env_path.parent,
    ...
    lines.append(f"papylonation_home:      {display_papylonation_home()}")
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(papylonation_home)}")
    lines.append(f"  skills:             {_count_skills(papylonation_home)}")
