    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                f"Files will be at: [cyan]{display_papylonation_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_papylonation_home()}/.env or run 'gh auth login'.\n")
            return
...
def skills_command(args) -> None:
    """Router for `hermes skills <subcommand>` — called from papylonation_cli/main.py."""
