...
# load_papylonation_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_papylonation_dotenv().
...
# process.  ``load_papylonation_dotenv()`` is called at module-import time from
# several hot modules (cli.py, papylonation_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_papylonation_dotenv()`` call.  Returns
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``papylonation_cli.config._sanitize_env_lines`` which
    ...
    if not path.exists():
        return
    try:
        from papylonation_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_papylonation_dotenv(
    *,
    papylonation_home: str | os.PathLike | None = None,
    ...
    home_path = Path(papylonation_home or os.getenv("HERMES_HOME", Path.home() / ".hermes"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    ``home_path`` are no-ops.  ``load_papylonation_dotenv()`` runs at import
    time from several hot modules (cli.py, papylonation_cli/main.py,
