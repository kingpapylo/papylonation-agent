...
# IMPORTANT: papylonation_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``papylonation_bootstrap`` is a
...
# new code references ``papylonation_bootstrap`` but the editable install's
...
# that run *before* papylonation_cli.config is importable. Mirrors the explicit
...
from papylonation_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from papylonation_cli.subcommands.cron import build_cron_parser
from papylonation_cli.subcommands.gateway import build_gateway_parser
from papylonation_cli.subcommands.profile import build_profile_parser
from papylonation_cli.subcommands.model import build_model_parser
from papylonation_cli.subcommands.setup import build_setup_parser
from papylonation_cli.subcommands.postinstall import build_postinstall_parser
from papylonation_cli.subcommands.whatsapp import build_whatsapp_parser
from papylonation_cli.subcommands.slack import build_slack_parser
from papylonation_cli.subcommands.login import build_login_parser
from papylonation_cli.subcommands.logout import build_logout_parser
from papylonation_cli.subcommands.auth import build_auth_parser
from papylonation_cli.subcommands.status import build_status_parser
from papylonation_cli.subcommands.webhook import build_webhook_parser
from papylonation_cli.subcommands.hooks import build_hooks_parser
from papylonation_cli.subcommands.doctor import build_doctor_parser
from papylonation_cli.subcommands.security import build_security_parser
from papylonation_cli.subcommands.dump import build_dump_parser
from papylonation_cli.subcommands.debug import build_debug_parser
from papylonation_cli.subcommands.backup import build_backup_parser
from papylonation_cli.subcommands.import_cmd import build_import_cmd_parser
from papylonation_cli.subcommands.config import build_config_parser
from papylonation_cli.subcommands.console import build_console_parser
from papylonation_cli.subcommands.version import build_version_parser
from papylonation_cli.subcommands.update import build_update_parser
from papylonation_cli.subcommands.uninstall import build_uninstall_parser
from papylonation_cli.subcommands.dashboard import build_dashboard_parser
from papylonation_cli.subcommands.gui import build_gui_parser
from papylonation_cli.subcommands.logs import build_logs_parser
from papylonation_cli.subcommands.prompt_size import build_prompt_size_parser
from papylonation_cli.subcommands.memory import build_memory_parser
from papylonation_cli.subcommands.acp import build_acp_parser
from papylonation_cli.subcommands.tools import build_tools_parser
from papylonation_cli.subcommands.insights import build_insights_parser
from papylonation_cli.subcommands.skills import build_skills_parser
from papylonation_cli.subcommands.pairing import build_pairing_parser
from papylonation_cli.subcommands.plugins import build_plugins_parser
from papylonation_cli.subcommands.mcp import build_mcp_parser
from papylonation_cli.subcommands.claw import build_claw_parser
...
def _apply_profile_override() -> None:
    ...
    # Mirrors papylonation_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # See issue #22502.
    papylonation_home_env = os.environ.get("HERMES_HOME", "")
    if profile_name is None and papylonation_home_env:
        if Path(papylonation_home_env).parent.name == "profiles":
            return
    ...
    if profile_name is None and not os.environ.get("HERMES_S6_SUPERVISED_CHILD"):
        try:
            ...
            active_path = get_default_papylonation_root() / "active_profile"
    ...
    if profile_name is not None:
        try:
            from papylonation_cli.profiles import resolve_profile_env

            papylonation_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            papylonation_home = _resolve_sudo_user_profile_env(profile_name)
            if not papylonation_home:
            ...
            return
        os.environ["HERMES_HOME"] = papylonation_home
...
load_papylonation_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE papylonation_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_papylonation_home() / "config.yaml"
    if _cfg_path.exists():
...
try:
    from papylonation_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from papylonation_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if papylonation_constants not importable yet
...
from papylonation_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to papylonation_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (papylonation_cli.main._model_flow_*) keep resolving unchanged.
from papylonation_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_papylonation_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from papylonation_cli.config import get_env_path, get_papylonation_home, load_config
    from papylonation_cli.auth import get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_papylonation_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_papylonation_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, papylonation_bin
    ...
    papylonation_bin = container_info["papylonation_bin"]
    ...
    exec_cmd = (
        ...
        + ["-u", exec_user]
        + env_flags
        + [container_name, papylonation_bin]
        + cli_args
    )
...
def _ensure_tui_node() -> None:
    ...
    papylonation_home = os.environ.get("HERMES_HOME") or str(Path.home() / ".hermes")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HERMES_HOME": papylonation_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(papylonation_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(papylonation_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if papylonation_cli_dir is None:
        papylonation_cli_dir = Path(__file__).parent
    bundled = papylonation_cli_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from papylonation_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from papylonation_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from papylonation_cli.config import apply_terminal_config_to_env
    ...
    if code == 42:
        from papylonation_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from papylonation_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    try:
        from papylonation_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from papylonation_cli.config import load_config as _load_config_for_xai_check
    ...
    if not _has_any_provider_configured():
        ...
        from papylonation_cli.setup import (
    ...
    if _termux_should_prefetch_update_check():
        try:
            from papylonation_cli.banner import prefetch_update_check
...
def cmd_proxy(args):
    ...
    from papylonation_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from papylonation_constants import find_node_executable, with_papylonation_node_path
    ...
    session_dir = get_papylonation_home() / "whatsapp" / "session"
...
def cmd_whatsapp_cloud(args):
    ...
    ``papylonation_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from papylonation_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from papylonation_cli.config import stamp_install_method
    from papylonation_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    from papylonation_cli.models import (
        CANONICAL_PROVIDERS,
        _PROVIDER_LABELS,
        group_providers,
    ...
    # PROVIDER_GROUPS in papylonation_cli/models.py). A multi-member group shows one
...
def _clear_stale_openai_base_url():
    ...
    """
    from papylonation_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`papylonation_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from papylonation_cli.config import load_config
    from papylonation_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from papylonation_cli.auth import _prompt_model_selection
    from papylonation_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from papylonation_cli.setup import _curses_prompt_choice
...
def _remove_custom_provider(config):
    ...
    try:
        from papylonation_cli.curses_ui import curses_radiolist
...
# downstream call sites read `papylonation_cli.main._PROVIDER_MODELS` directly,
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from papylonation_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """
    from papylonation_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from papylonation_cli.env_loader import format_secret_source_suffix
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from papylonation_constants import display_papylonation_home as _dhh_fn
...
def cmd_login(args):
    ...
    from papylonation_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from papylonation_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from papylonation_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from papylonation_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from papylonation_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from papylonation_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from papylonation_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from papylonation_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from papylonation_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from papylonation_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from papylonation_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from papylonation_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from papylonation_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from papylonation_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from papylonation_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from papylonation_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from papylonation_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from papylonation_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from papylonation_cli.config import detect_install_method
    ...
    try:
        from papylonation_cli.banner import check_for_updates
        from papylonation_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from papylonation_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from papylonation_cli.uninstall import run_gui_uninstall
    ...
    from papylonation_cli.uninstall import run_uninstall
...
_UPDATE_CRITICAL_FILES = (
    "papylonation_cli/main.py",
    "papylonation_cli/config.py",
    "papylonation_cli/__init__.py",
    "papylonation_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "papylonation_constants.py",
)
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``papylonation_cli/web_dist/`` (per vite.config.ts
    outDir: "../papylonation_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "papylonation_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``hermes`` launcher present but ``papylonation_cli`` not importable.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m papylonation_cli.main web` invocation.
    ...
    from papylonation_constants import with_papylonation_node_path
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_papylonation_node_path()
...
def _desktop_stamp_path() -> Path:
    ...
    from papylonation_constants import get_papylonation_home
    return get_papylonation_home() / "desktop-build-stamp.json"
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_papylonation_node_path(env)
    if mirror:
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from papylonation_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "papylonation_root", None):
        env["HERMES_DESKTOP_HERMES_ROOT"] = str(Path(args.papylonation_root).expanduser().resolve())
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            nixos_env = with_papylonation_node_path(_nixos_build_env())
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    patterns = [
        "hermes dashboard",
        "papylonation_cli.main dashboard",
        "papylonation_cli/main.py dashboard",
        ...
        "hermes serve",
        "papylonation_cli.main serve",
        "papylonation_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `papylonation_cli.gateway._scan_gateway_pids` and avoids the
...
def _update_via_zip(args):
    ...
    from papylonation_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from papylonation_cli.model_catalog import seed_cache_from_checkout
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_papylonation_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_papylonation_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    default_home = get_default_papylonation_root()
...
def _recover_from_interrupted_install() -> None:
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _papylonation_exe_shims(scripts_dir)
            if shims:
    ...
    try:
        ...
        try:
            from papylonation_cli.managed_uv import ensure_uv
...
def _papylonation_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_papylonation_instances(
    ...
    for shim in _papylonation_exe_shims(scripts_dir):
        try:
...
def _quarantine_running_papylonation_exe(
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_papylonation_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_papylonation_exe(scripts_dir)
    try:
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    ``_quarantine_running_papylonation_exe`` for the rationale.
    """
    ...
    *,
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m papylonation_cli.main <command>"
        )
    else:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from papylonation_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    """
    from papylonation_cli.managed_uv import resolve_uv
...
def _npm_lockfile_changed(papylonation_root: Path) -> bool:
    ...
    try:
        ...
        cache_file = papylonation_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(papylonation_root: Path) -> None:
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from papylonation_constants import find_node_executable
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from papylonation_constants import is_wsl
    ...
    shared_papylonation_root = get_default_papylonation_root()
    if not _npm_lockfile_changed(shared_papylonation_root):
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_papylonation_root)
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # papylonation_cli.config.get_papylonation_home to simulate setup failure.
        from papylonation_cli.config import get_papylonation_home as _get_papylonation_home

        logs_dir = _get_papylonation_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "docker":
        ...
        from papylonation_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from papylonation_cli.config import recommended_update_command
        from papylonation_cli.banner import check_via_pypi
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    snapshot_id = None
    try:
        from papylonation_cli.backup import create_quick_snapshot
    ...
    try:
        from papylonation_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_papylonation_home so the user sees ~/.hermes/...
    try:
        from papylonation_constants import get_papylonation_home, display_papylonation_home

        home = get_papylonation_home()
        try:
            display_path = f"{display_papylonation_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    Windows: the Desktop app's backend (``python.exe -m papylonation_cli.main
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m papylonation_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "papylonation_cli.main" in cmdline_low:
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    try:
        ...
        from papylonation_cli.gateway import (
    ...
    if not running_pids:
        ...
        try:
            from papylonation_cli import gateway_windows
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m papylonation_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from papylonation_cli import gateway_windows
        from papylonation_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _cmd_update_pip(args):
    ...
    from papylonation_cli.config import is_uv_tool_install
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_papylonation_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # orphan merge-conflict markers in papylonation_cli/config.py bricked
        ...
        # the old bytecode (e.g. get_papylonation_home added to papylonation_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        if (desktop_dir / "package.json").exists() and _resolve_node_runtime_npm() and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "papylonation_cli.main", "desktop", "--build-only"]
            ...
            _build_env = with_papylonation_node_path()
            ...
            if build_result.returncode != 0:
                ...
                if tail:
                    print(tail)
                from papylonation_constants import display_papylonation_home as _dhh
        ...
        # modules in this process.  Reload papylonation_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_papylonation_home() added since the last release.
        try:
            import importlib
            import papylonation_constants as _hc
        ...
        try:
            from papylonation_cli.profiles import backfill_profile_envs
        ...
        # it and warn loudly.
        try:
            from papylonation_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            ...
            if (
                ...
                from papylonation_cli.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_papylonation_home() / ".update_exit_code"
            try:
        ...
        try:
            from papylonation_cli.gateway import (
                has_legacy_papylonation_units,
                _find_legacy_papylonation_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_papylonation_units():
                print()
                print("⚠ Legacy Hermes gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_papylonation_units():
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from papylonation_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from papylonation_constants import get_papylonation_home as _hh
        ...
        from papylonation_cli import profile_describer as _pd
        ...
        from papylonation_cli.profiles import profile_exists, validate_alias_name
        ...
        from papylonation_cli.profiles import rename_profile
        ...
        from papylonation_cli.profiles import export_profile
        ...
        from papylonation_cli.profiles import import_profile
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="papylonation_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        from papylonation_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from papylonation_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("papylonation_requires"):
            print(f"Requires:     Hermes {data['papylonation_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from papylonation_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.papylonation_requires:
        print(f"  Requires: Hermes {mf.papylonation_requires}")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from papylonation_cli.web_server import should_require_auth
    ...
    try:
        from papylonation_cli.dashboard_auth import list_providers
        if list_providers():
...
def cmd_dashboard(args):
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "papylonation_cli.main",
            "-p", "default",
        ...
        # update-button guard also misfires).  get_default_papylonation_root()
        ...
        try:
            from papylonation_constants import get_default_papylonation_root
            env["HERMES_HOME"] = str(get_default_papylonation_root())
        except Exception:
    ...
    if _headless_backend:
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "papylonation_cli" / "web_dist"
        )
    ...
    try:
        from papylonation_cli.mcp_startup import start_background_mcp_discovery
    ...
    from papylonation_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from papylonation_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from papylonation_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from papylonation_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from papylonation_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from papylonation_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from papylonation_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from papylonation_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``papylonation_cli/_parser.py``.
#
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if getattr(args, "oneshot", None):
        ...
        from papylonation_cli.oneshot import run_oneshot
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_papylonation_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_papylonation_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_papylonation_home()}/memories/\n")
    else:
        from papylonation_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from papylonation_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from papylonation_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from papylonation_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from papylonation_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from papylonation_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from papylonation_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from papylonation_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from papylonation_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from papylonation_cli.claw import claw_command
...
def main():
    ...
    # there's nothing to clean. See ``_quarantine_running_papylonation_exe``.
    try:
    ...
    # =========================================================================
    # model command  (parser built in papylonation_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from papylonation_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from papylonation_cli.fallback_cmd import cmd_fallback
    ...
    from papylonation_cli import secrets_cli as _secrets_cli
    from papylonation_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from papylonation_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in papylonation_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in papylonation_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in papylonation_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in papylonation_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in papylonation_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from papylonation_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in papylonation_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in papylonation_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in papylonation_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in papylonation_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in papylonation_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in papylonation_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from papylonation_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from papylonation_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from papylonation_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in papylonation_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in papylonation_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in papylonation_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in papylonation_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in papylonation_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in papylonation_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from papylonation_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in papylonation_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in papylonation_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in papylonation_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in papylonation_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in papylonation_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from papylonation_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in papylonation_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from papylonation_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from papylonation_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from papylonation_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from papylonation_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in papylonation_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in papylonation_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from papylonation_cli.tools_config import _cua_driver_cmd
            ...
            if path:
                version = ""
                try:
                    from papylonation_cli.tools_config import _cua_driver_env
    ...
    # =========================================================================
    # mcp command  (parser built in papylonation_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def cmd_sessions(args):
        ...
        if action == "repair":
            from papylonation_state import (
                DEFAULT_DB_PATH,
                _db_opens_cleanly,
        ...
        if action == "list":
            from papylonation_state import workspace_key as _ws_key
            ...
            from papylonation_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from papylonation_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from papylonation_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_papylonation_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from papylonation_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_papylonation_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_papylonation_home() / "sessions"
    ...
    # =========================================================================
    # insights command  (parser built in papylonation_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in papylonation_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in papylonation_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in papylonation_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in papylonation_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in papylonation_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in papylonation_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in papylonation_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in papylonation_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in papylonation_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in papylonation_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    from papylonation_cli.config import get_container_exec_info
