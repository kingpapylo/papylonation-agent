...
Usage:
    python -m papylonation_cli.main web          # Start on http://127.0.0.1:9119
    python -m papylonation_cli.main web --port 8080
"""
...
from papylonation_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from papylonation_cli.memory_providers import (
...
def _warm_gateway_module() -> None:
    try:
        import papylonation_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from papylonation_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire papylonation_cli.gateway import into a background thread so the event
...
from papylonation_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
#
# This list is defined in ``papylonation_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from papylonation_cli.dashboard_auth.public_paths import (
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from papylonation_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from papylonation_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from papylonation_cli.dashboard_auth.token_auth import token_auth_middleware
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from papylonation_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from papylonation_cli.model_normalize import normalize_model_for_provider
    from papylonation_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from papylonation_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    """
    from papylonation_state import DEFAULT_DB_PATH, SessionDB
...
def _default_papylonation_root_is_opt_data() -> bool:
    ...
    try:
        ...
        root = get_default_papylonation_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    """
    if _default_papylonation_root_is_opt_data():
        return True
    try:
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    with _profile_scope(profile) as scoped_home:
        home = scoped_home or get_papylonation_home()
...
# gate + path hardening as /api/fs). Logic lives in ``papylonation_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from papylonation_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from papylonation_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_papylonation_home() at call time
    ...
    try:
        ...
        # Windows install the first import of papylonation_cli.gateway blocks the
        ...
        try:
            from papylonation_cli.dashboard_auth import list_providers as _list_providers
        ...
        try:
            from papylonation_cli.auth import get_nous_session_validity
...
async def get_system_stats():
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "papylonation_version": __version__,
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_papylonation_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_papylonation_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_papylonation_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_papylonation_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_papylonation_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from papylonation_cli.debug import build_debug_share
...
_ACTION_LOG_DIR: Path = get_papylonation_home() / "logs"
...
def _spawn_papylonation_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``hermes <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``papylonation_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "papylonation_cli.main", *subcommand]
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_papylonation_action(subcommand, "gateway-restart"), False
...
async def update_hermes():
    ...
    try:
        proc = _spawn_papylonation_action(["update"], "hermes-update")
    except Exception as exc:
...
async def check_papylonation_update(force: bool = False):
    ...
    try:
        ...
        if force:
            try:
                (get_papylonation_home() / ".update_check").unlink()
            except OSError:
                pass
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        papylonation_home / f"{name}.json",
        papylonation_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_papylonation_home()))
            return
...
# ---------------------------------------------------------------------------
# Declared surface — curated desktop schema from papylonation_cli.memory_providers.
...
def _declared_provider_file_path(provider: DeclaredMemoryProvider) -> Path:
    return get_papylonation_home() / provider.name / "config.json"
...
# in papylonation_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(
    ...
    """
    try:
        from papylonation_cli.inventory import build_models_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        from papylonation_cli.inventory import build_models_payload, load_picker_context
        from papylonation_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from papylonation_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from papylonation_cli.moa_config import normalize_moa_config, validate_moa_payload
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (papylonation_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from papylonation_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from papylonation_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    try:
        from papylonation_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
def _whatsapp_session_path() -> Path:
    ...
    return get_papylonation_dir("platforms/whatsapp/session", "whatsapp/session")
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_papylonation_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_papylonation_oauth_credentials,
            _get_papylonation_oauth_file,
        )
    except ImportError:
        read_papylonation_oauth_credentials = None  # type: ignore
        _get_papylonation_oauth_file = None  # type: ignore

    papylonation_creds = None
    if read_papylonation_oauth_credentials:
        try:
            papylonation_creds = read_papylonation_oauth_credentials()
        except Exception:
            papylonation_creds = None
    if papylonation_creds and papylonation_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "papylonation_pkce",
            "source_label": f"Hermes PKCE ({_get_papylonation_oauth_file() if _get_papylonation_oauth_file else None})",
            "token_preview": _truncate_token(papylonation_creds.get("accessToken")),
            "expires_at": papylonation_creds.get("expiresAt"),
            "has_refresh_token": bool(papylonation_creds.get("refreshToken")),
        }
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from papylonation_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("papylonation_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_papylonation_oauth_file
            ...
            try:
                from papylonation_cli.auth import clear_provider_auth
        ...
        try:
            from papylonation_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from papylonation_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from papylonation_cli.auth import _save_codex_tokens
...
def _prune_sessions(body: SessionPrune):
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_papylonation_home()
    db = _open_session_db_for_profile(body.profile)
    try:
        filters = dict(
...
async def get_logs(
    ...
    from papylonation_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_papylonation_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from papylonation_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["papylonation_home"] = str(home)
...
#
# Wraps the same config data layer the CLI uses (papylonation_cli.mcp_config), so
...
def _normalize_mcp_server_create(
    ...
    """
    from papylonation_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from papylonation_cli.mcp_security import validate_mcp_server_entry
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from papylonation_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from papylonation_cli.mcp_config import _remove_mcp_server
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from papylonation_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.papylonation_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        home_token = set_papylonation_home_override(flow.papylonation_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.papylonation_home)))
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        papylonation_home=flow.papylonation_home,
                    )
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    process_home = str(get_papylonation_home().expanduser().resolve(strict=False))
    with _profile_scope(profile):
        servers = _get_mcp_servers()
        flow_home = str(get_papylonation_home().expanduser().resolve(strict=False))
    if name not in servers:
        raise HTTPException(status_code=404, detail=f"Server '{name}' not found")
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(
            ...
            and flow.papylonation_home == flow_home
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    """
    try:
        from papylonation_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    if entry.install is not None:
        ...
        try:
            proc = _spawn_papylonation_action(
...
#
# Wraps the same JSON store the CLI uses (papylonation_cli.webhook); the webhook
...
async def list_webhooks():
    import papylonation_cli.webhook as wh
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_papylonation_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_papylonation_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def run_doctor():
    try:
        proc = _spawn_papylonation_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_papylonation_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_papylonation_home() / "backups"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_papylonation_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_papylonation_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    try:
        from papylonation_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <papylonation_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_papylonation_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_papylonation_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_papylonation_action`` tracks one process/log per name, so a shared
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from papylonation_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_papylonation_home()
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    token = set_papylonation_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from papylonation_constants import set_papylonation_home_override, reset_papylonation_home_override
    from papylonation_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    """
    from papylonation_constants import set_papylonation_home_override, reset_papylonation_home_override
    from papylonation_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_papylonation_home()`` at call
       time — the context-local override from ``set_papylonation_home_override``
    ...
    retargeted to the *current* ``get_papylonation_home()`` so writes land in the
    ...
    token = None
    if not requested or requested.lower() == "current":
        profile_dir = get_papylonation_home()
    else:
        profile_dir = _resolve_profile_dir(requested)
        token = set_papylonation_home_override(str(profile_dir))
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_papylonation_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_papylonation_home()`` at call time
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from papylonation_cli.platforms import platform_label
...
if sys.platform.startswith("win"):
    try:
        from papylonation_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from papylonation_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from papylonation_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from papylonation_cli.dashboard_auth.audit import AuditEvent, audit_log
        from papylonation_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    matching what ``papylonation_cli.main._launch_tui`` does for the CLI path.
    ...
    ``get_papylonation_home()`` from that env var at its own import, so the child
    ...
    """
    from papylonation_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from papylonation_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from papylonation_cli.console_engine import HermesConsoleEngine
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`papylonation_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from papylonation_cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_papylonation_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as papylonation_cli.plugins):
    ...
    from papylonation_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_papylonation_home() / "plugins", "user"),
    ...
    # ``papylonation_cli/plugins.py`` and the documented user contract.
...
async def get_dashboard_plugins():
    ...
    try:
        from papylonation_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_papylonation_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from papylonation_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from papylonation_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from papylonation_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from papylonation_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"papylonation_dashboard_plugin_{plugin['name']}"
...
from papylonation_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            try:
                ...
                from papylonation_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
