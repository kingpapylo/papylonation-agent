...
# display_papylonation_home imported lazily at call sites (stale-module safety during hermes update)
...
from papylonation_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from papylonation_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ PapyloNation Agent Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure PapyloNation Agent using environment variables or config commands:")
...
def is_noninteractive() -> bool:
    ...
    ``HERMES_NONINTERACTIVE=1`` (see ``papylonation_cli/web_server.py``). In that
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from papylonation_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, papylonation_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from papylonation_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from papylonation_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {papylonation_home}/cron/, sessions/, logs/"
    )
    print()
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from papylonation_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    """
    try:
        from papylonation_cli.auth import get_xai_oauth_auth_status
...
def _setup_telegram_auto_result():
    ...
    try:
        from papylonation_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_papylonation_home(Path(get_papylonation_home()))
    except Exception:
        pass
...
def _profile_name_from_papylonation_home(papylonation_home) -> str | None:
    """Return the active profile name when HERMES_HOME is a profile dir."""
    if papylonation_home.parent.name == "profiles":
        return papylonation_home.name
    return None
...
def _setup_qqbot():
    ...
    from papylonation_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from papylonation_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_papylonation_units():
            print_legacy_unit_warning()
            print()
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``papylonation_cli.auth`` — lists every supported
    ...
    """
    try:
        from papylonation_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from papylonation_cli.config
    ...
    if section_key == "model":
        ...
        from papylonation_cli.gateway import _all_platforms, _platform_status
...
def _offer_openclaw_migration(papylonation_home: Path) -> bool:
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=papylonation_home.resolve(),
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from papylonation_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from papylonation_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, papylonation_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(papylonation_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, papylonation_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, papylonation_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {papylonation_home}")
    ...
    _print_setup_summary(config, papylonation_home)


def _run_first_time_quick_setup(config: dict, papylonation_home, is_existing: bool):
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from papylonation_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _run_blank_slate_setup(config: dict, papylonation_home, is_existing: bool):
    ...
    _blank_slate_walkthrough(config, papylonation_home)


def _blank_slate_walkthrough(config: dict, papylonation_home):
...
def _run_quick_setup(config: dict, papylonation_home):
