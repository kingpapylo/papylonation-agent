...
_PROFILE_DIRS = [
    ...
    # See papylonation_constants.get_subprocess_home().
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_papylonation_home().resolve()
...
# (#58394). Add new artifacts here when introduced in ``papylonation_constants``.
...
def _get_profiles_root() -> Path:
    ...
    """
    return _get_default_papylonation_home() / "profiles"


def _get_default_papylonation_home() -> Path:
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_papylonation_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    ...
    if canon == "default":
        return _get_default_papylonation_home()
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            papylonation_exe = shutil.which("hermes") or "hermes"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(papylonation_exe)} -p {profile} "$@"\n')
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_papylonation_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, papylonation_home)`` pairs a gateway should serve.
    ...
    The returned ``papylonation_home`` is the path to pass to
    ``set_papylonation_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_papylonation_home())]
...
def create_profile(
    ...
    if clone_from is not None or clone_all or clone_config:
        if clone_from is None:
            ...
            source_dir = get_papylonation_home()
        else:
    ...
    if not soul_path.exists():
        try:
            from papylonation_cli.default_soul import DEFAULT_SOUL_MD
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_papylonation_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    papylonation_markers = ("papylonation_cli.main", "hermes-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            is_hermes = (
                any(marker in joined for marker in papylonation_markers)
                or exe_name == "hermes"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_papylonation_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from papylonation_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from papylonation_cli.gateway import get_service_name, get_launchd_plist_path
...
def get_active_profile_name() -> str:
    ...
    resolved = papylonation_home.resolve()

    default_resolved = _get_default_papylonation_home().resolve()
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="papylonation_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"papylonation_{old_name}"
    legacy_old_host = f"hermes.{old_name}"
    new_host = f"papylonation_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_papylonation_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("papylonation_"):
