...
from papylonation_cli.urllib_security import open_credentialed_url
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from papylonation_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_papylonation_home() / "cache" / "nous_recommended_cache.json"
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from papylonation_cli.model_catalog import get_default_model_from_cache
...
# ``get_recommended_default_model`` in papylonation_cli/web_server.py and
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from papylonation_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from papylonation_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from papylonation_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from papylonation_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``papylonation_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from papylonation_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from papylonation_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _provider_models_cache_path() -> Path:
    from papylonation_constants import get_papylonation_home
    return get_papylonation_home() / "provider_models_cache.json"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from papylonation_constants import get_papylonation_home
    return get_papylonation_home() / "ollama_cloud_models_cache.json"
