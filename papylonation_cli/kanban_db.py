...
from papylonation_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "papylonation_kanban_current_board_override",
    default=None,
)
...
def kanban_home() -> Path:
    ...
    2. ``get_default_papylonation_root()``, which already returns ``<root>``
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (papylonation_cli/projects_db). When set,
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See papylonation_state._WAL_INCOMPAT_MARKERS for detection logic.
                from papylonation_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from papylonation_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    if project_id:
        try:
            from papylonation_cli import projects_db as _pdb
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from papylonation_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from papylonation_cli.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from papylonation_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_papylonation_argv() -> list[str]:
    """Return the interpreter-bound Hermes CLI invocation."""
    # ``papylonation_cli.main`` is the console-script target declared in
...
def _absolute_papylonation_path(path: str) -> str:
...
def _papylonation_path_argv(path: str) -> list[str]:
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_papylonation_argv()
    return [_absolute_papylonation_path(path)]


def _resolve_papylonation_argv() -> list[str]:
    ...
    3. ``sys.executable -m papylonation_cli.main`` — fallback for setups where
       Hermes is launched from a venv and the ``hermes`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_papylonation_bin`` for the same reason. Kept
    local (not imported from gateway) because ``papylonation_cli`` sits below
    ...
    if env_bin:
        if _looks_like_path(env_bin):
            return _papylonation_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _papylonation_path_argv(resolved_env_bin)
        return _module_papylonation_argv()

    papylonation_bin = _safe_which_no_cwd("hermes") if _IS_WINDOWS else shutil.which("hermes")
    if papylonation_bin:
        return _papylonation_path_argv(papylonation_bin)
    return _module_papylonation_argv()
...
def _resolve_worker_cli_toolsets(papylonation_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_papylonation_home_override(papylonation_home)
        try:
            cfg = load_config()
...
def _default_spawn(
    ...
    *,
    ...
    # _apply_profile_override() runs *before* papylonation_constants is imported.
    # If HERMES_HOME is absent from the child's env, get_papylonation_home() falls
    ...
    # dispatcher's. Belt-and-braces with the `get_default_papylonation_root()`
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``papylonation_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from papylonation_constants import get_default_papylonation_root
