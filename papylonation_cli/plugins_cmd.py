...
def _plugins_dir() -> Path:
    ...
    plugins = get_papylonation_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from papylonation_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_papylonation_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_papylonation_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_papylonation_home()}/.env later)[/dim]")
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``papylonation_agent.plugins``.
    ...
    """
    from papylonation_cli.plugins import ENTRY_POINTS_GROUP
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        from papylonation_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
