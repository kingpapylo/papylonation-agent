...
- ``papylonation_cli.providers``        -- canonical provider identity + overlays
- ``papylonation_cli.model_normalize``  -- per-provider name formatting
...
from papylonation_cli.providers import (
    ProviderDef,
...
def is_nous_papylonation_non_agentic(model_name: str) -> bool:
...
def _check_papylonation_model_warning(model_name: str) -> str:
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from papylonation_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                from papylonation_cli.config import validate_config_structure
        ...
        from papylonation_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from papylonation_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from papylonation_cli.config import load_config
            from papylonation_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from papylonation_cli.providers import resolve_user_provider as _ruser
    ...
    # papylonation_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from papylonation_cli.models import normalize_opencode_base_url
    ...
    papylonation_warn = _check_papylonation_model_warning(new_model)
    if papylonation_warn:
        warnings.append(papylonation_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from papylonation_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from papylonation_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from papylonation_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from papylonation_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    if "ollama-cloud" not in curated:
        from papylonation_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from papylonation_cli.models import fetch_lmstudio_models
    ...
    from papylonation_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from papylonation_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for papylonation_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(papylonation_id)
        if (
            _alias_target
            and _alias_target != papylonation_id
        ...
        pconfig = PROVIDER_REGISTRY.get(papylonation_id)
        ...
        from papylonation_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(papylonation_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(papylonation_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        papylonation_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        model_ids = cached_provider_model_ids(papylonation_id)
        if not model_ids:
            model_ids = curated.get(papylonation_id, [])
            if papylonation_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(papylonation_id, model_ids)
    ...
    from papylonation_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        papylonation_slug = _mdev_to_hermes.get(pid, pid)
        if papylonation_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(papylonation_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, papylonation_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or papylonation_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(papylonation_slug):
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", papylonation_slug, exc)
        ...
        # configured.
        if not has_creds and papylonation_slug == "anthropic":
            try:
                ...
                if (papylonation_creds and papylonation_creds.get("accessToken")) or \
        ...
        if papylonation_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(papylonation_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(papylonation_slug)
                model_ids = _ids if _ids else (curated.get(papylonation_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(papylonation_slug, []) or curated.get(pid, [])
        elif papylonation_slug == "nous":
            ...
            try:
                ...
                from papylonation_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(papylonation_slug, []) or curated.get(pid, [])
                if papylonation_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(papylonation_slug, model_ids)
        total = len(model_ids)
        if papylonation_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids

        results.append({
            "slug": papylonation_slug,
            "name": get_label(papylonation_slug),
            "is_current": papylonation_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "hermes",
        ...
        seen_slugs.add(papylonation_slug.lower())
        _record_builtin_endpoint(papylonation_slug)
    ...
    # with `hermes model`).
    try:
        from papylonation_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            if should_probe:
                try:
                    from papylonation_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, papylonation_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from papylonation_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`papylonation_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from papylonation_cli.models import fetch_openrouter_models
