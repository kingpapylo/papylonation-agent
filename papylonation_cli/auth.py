...
from papylonation_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from papylonation_cli import __version__ as _HERMES_CLI_VERSION
...
def get_anthropic_key() -> str:
    ...
    """
    from papylonation_cli.config import get_env_value_prefer_dotenv
...
def _auth_file_path() -> Path:
    path = get_papylonation_home() / "auth.json"
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_papylonation_root()
    except Exception:
        return None
    profile_home = get_papylonation_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``get_default_papylonation_root()`` for a profile-shaped HERMES_HOME can
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "papylonation_pkce", "manual"}
...
# ``get_default_papylonation_root()`` returns — ``~/.hermes`` on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`papylonation_constants.get_default_papylonation_root` returns — so
    ...
    from papylonation_constants import get_default_papylonation_root
    return get_default_papylonation_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from papylonation_constants import get_default_papylonation_root
        real_home_shared = (
            get_default_papylonation_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from papylonation_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _prompt_model_selection(
    ...
    """
    from papylonation_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from papylonation_cli.config import save_config, load_config
