...
from papylonation_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_papylonation_home() / ".managed"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``papylonation_cli/`` — i.e. the git checkout for source
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    try:
        method = (
            (get_papylonation_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``papylonation_constants.is_container`` (import-safe)."""
    try:
...
# papylonation_cli/main.py; lives here so the wording stays consistent and we
...
def get_container_exec_info() -> Optional[dict]:
    ...
    Returns a dict with keys: backend, container_name, exec_user, papylonation_bin
    ...
    container_mode_file = get_papylonation_home() / ".container-mode"
    ...
    papylonation_bin = info.get("papylonation_bin", "/data/current-package/bin/hermes")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "papylonation_bin": papylonation_bin,
    }
...
# Re-export from papylonation_constants — canonical definition lives there.
from papylonation_constants import get_papylonation_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_papylonation_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_papylonation_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_papylonation_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    subdirectories created at runtime by ``ensure_papylonation_home()`` (especially
...
def _chown_to_papylonation_uid(path) -> None:
    ...
    directories created by :func:`ensure_papylonation_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_papylonation_uid_gid()
...
def _secure_dir(path):
    ...
    try:
        ...
        pass
    _chown_to_papylonation_uid(path)
...
def ensure_papylonation_home():
    ...
    if is_managed():
        ...
        try:
            _ensure_papylonation_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_papylonation_home_managed(home: Path):
...
DEFAULT_CONFIG = {
    ...
    # See tools/kanban_tools.py and papylonation_cli/kanban_db.py for the actual
    ...
    "display": {
        ...
        "pet": {
            ...
            # get_papylonation_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "moa": {
        ...
        # <papylonation_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    "security": {
        ...
        # ``papylonation_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``papylonation_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    #      on top of ensure_papylonation_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_papylonation_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from papylonation_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from papylonation_cli.toolset_validation import validate_platform_toolsets
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_papylonation_dotenv()`` populated the process env, or an env var
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _papylonation_home / "config.yaml"`` local variable
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_papylonation_dotenv()
