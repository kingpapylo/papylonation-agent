...
def _translate_one_server(
    name: str, papylonation_cfg: dict
    ...
    if not isinstance(papylonation_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(papylonation_cfg.get("command"))
    has_url = bool(papylonation_cfg.get("url"))
...
def _build_papylonation_tools_mcp_entry() -> dict:
    ...
    # (not get_papylonation_home()) on purpose: when the env var is unset we want
    ...
    papylonation_home = os.environ.get("HERMES_HOME") or ""
    if papylonation_home and _looks_like_test_tempdir(papylonation_home):
        papylonation_home = ""
    if papylonation_home:
        env["HERMES_HOME"] = papylonation_home
    ...
    out: dict[str, Any] = {
        ...
        "args": ["-m", "agent.transports.papylonation_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    Args:
        ...
        expose_papylonation_tools: when True (default), register Hermes' own
    ...
    papylonation_servers = (papylonation_config or {}).get("mcp_servers") or {}
    if not isinstance(papylonation_servers, dict):
    ...
    for name, cfg in papylonation_servers.items():
    ...
    # The server itself is agent/transports/papylonation_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_papylonation_tools:
        translated["hermes-tools"] = _build_papylonation_tools_mcp_entry()
