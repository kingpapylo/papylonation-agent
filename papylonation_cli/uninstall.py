...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains papylonation_cli reference)
                content = wrapper.read_text()
                if 'papylonation_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(papylonation_home: Path) -> list:
    ...
    """
    node_dir = (papylonation_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    try:
        from papylonation_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from papylonation_cli.gateway import get_launchd_plist_path
...
def _papylonation_path_markers(papylonation_home: Path) -> list[str]:
    """Path-entry substrings that identify Hermes-owned User-PATH entries."""
    root = str(papylonation_home).rstrip("\\/")
...
def remove_path_from_windows_registry(papylonation_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _papylonation_path_markers(papylonation_home)
            kept: list[str] = []
...
def remove_papylonation_env_vars_windows() -> list[str]:
...
def remove_portable_tooling_windows(papylonation_home: Path) -> list[Path]:
...
def _is_default_papylonation_home(papylonation_home: Path) -> bool:
    """Return True when ``papylonation_home`` points at the default (non-profile) root."""
    try:
        from papylonation_constants import get_default_papylonation_root
        return papylonation_home.resolve() == get_default_papylonation_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m papylonation_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    papylonation_invocation = [_sys.executable, "-m", "papylonation_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                papylonation_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
...
def run_gui_uninstall(args):
    ...
    """
    from papylonation_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(papylonation_home)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {papylonation_home}, and the standard app locations for this OS.")
        return
    ...
    print()
    if agent_is_installed(papylonation_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Hermes agent at {papylonation_home / 'hermes-agent'}")
        print(f"  • Your config, sessions, and secrets under {papylonation_home}")
        print()
    ...
    print()
    uninstall_gui(papylonation_home)
...
def run_uninstall(args):
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            ...
            papylonation_home=papylonation_home,
    ...
    is_default_profile = _is_default_papylonation_home(papylonation_home)
    ...
    print(f"  Config:  {papylonation_home / 'config.yaml'}")
    print(f"  Secrets: {papylonation_home / '.env'}")
    print(f"  Data:    {papylonation_home / 'cron/'}, {papylonation_home / 'sessions/'}, {papylonation_home / 'logs/'}")
    print()
...
def _print_uninstall_dry_run(*, project_root: Path, papylonation_home: Path, full_uninstall: bool) -> None:
    ...
    if full_uninstall:
        print(f"  • Hermes config/data: {papylonation_home}")
        if _is_default_papylonation_home(papylonation_home):
        ...
        print(f"  • Keep Hermes config/data: {papylonation_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in papylonation_home so the marker matching is
        ...
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(papylonation_home))))
        ...
        removed_env = remove_papylonation_env_vars_windows()
        if removed_env:
    ...
    removed_node_links = remove_node_symlinks(papylonation_home)
    ...
    #     step-5 rmtree(papylonation_home) would sweep the in-tree artifacts anyway,
    ...
    try:
        from papylonation_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(papylonation_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            ...
            if papylonation_home in project_root.parents or project_root.parent == papylonation_home:
    ...
    #     the step-5 rmtree(papylonation_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(papylonation_home)
    ...
    if full_uninstall:
        ...
        try:
            if papylonation_home.exists():
                shutil.rmtree(papylonation_home)
                log_success(f"Removed {papylonation_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {papylonation_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {papylonation_home}")
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m papylonation_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import papylonation_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``papylonation_constants`` + ``papylonation_cli.colors``
    (and lazily ``papylonation_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m papylonation_cli.uninstall")
    parser.add_argument(
        "--mode",
