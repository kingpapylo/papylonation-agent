...
from papylonation_cli._parser import (
...
def resolve_papylonation_bin() -> Optional[str]:
    """Find the hermes entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("hermes")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m papylonation_cli.main``.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m papylonation_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    *,
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_papylonation_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "papylonation_cli.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m papylonation_cli.main``) or a ``.cmd`` batch file, and both
