...
from papylonation_constants import get_default_papylonation_root, get_papylonation_home, display_papylonation_home
...
def run_backup(args) -> None:
    ...
    papylonation_root = get_default_papylonation_root()

    if not papylonation_root.is_dir():
        print(f"Error: Hermes home directory not found at {papylonation_root}")
        sys.exit(1)
    ...
    # Collect files
    print(f"Scanning {display_papylonation_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(papylonation_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(papylonation_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(papylonation_root)
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_papylonation_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_papylonation_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (papylonation_root / "config.yaml").exists()
        has_env = (papylonation_root / ".env").exists()
        ...
        for member in members:
            ...
            target = papylonation_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(papylonation_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_papylonation_home()}."
            )
        ...
        profiles_dir = papylonation_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # papylonation_cli.profiles might not be available (fresh install)
        ...
        # Guidance
        print()
        if not (papylonation_root / "hermes-agent").is_dir():
...
def _quick_snapshot_root(papylonation_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(papylonation_home)
    if not root.exists():
        return []
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        papylonation_home: Override for the Hermes home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(papylonation_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_papylonation_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, papylonation_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``papylonation_root`` to ``out_path``.
...
def _pre_update_backup_dir(papylonation_home: Optional[Path] = None) -> Path:
    home = papylonation_home or get_papylonation_home()
...
def create_pre_update_backup(
    ...
    """
    papylonation_root = papylonation_home or get_default_papylonation_root()
    if not papylonation_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(papylonation_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, papylonation_root)
    if result is None:
        return None
