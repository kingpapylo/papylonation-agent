...
def _container_no_volume_mount(papylonation_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    home = papylonation_home or Path(
...
def run_security_audit(
    *, papylonation_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(papylonation_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(papylonation_home=papylonation_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
