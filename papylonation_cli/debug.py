...
def _pending_file() -> Path:
    ...
    """
    return get_papylonation_home() / "pastes" / "pending.json"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from papylonation_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_papylonation_home() / "logs" / filename) if filename else None
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    ...
    suggests ``--local``.
    """
    from papylonation_cli.diagnostics_upload import share_to_nous
