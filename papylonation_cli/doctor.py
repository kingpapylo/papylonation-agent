...
from papylonation_cli.config import get_project_root, get_papylonation_home, get_env_path
...
from papylonation_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
HERMES_HOME = get_papylonation_home()
_DHH = display_papylonation_home()  # user-facing display path (e.g. ~/.hermes or ~/.hermes/profiles/coder)

# Load environment variables from ~/.hermes/.env so API key checks work
_env_path = get_env_path()
load_papylonation_dotenv(papylonation_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from papylonation_cli.colors import Colors, color
from papylonation_cli.models import _HERMES_USER_AGENT
from papylonation_constants import OPENROUTER_MODELS_URL
...
from papylonation_constants import is_termux as _is_termux
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from papylonation_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches papylonation_cli.__version__.
    ...
    """
    try:
        from papylonation_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != papylonation_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'hermes update', or set "
            "papylonation_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from papylonation_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # Detect drift between pyproject.toml and papylonation_cli/__init__.py versions
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from papylonation_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from papylonation_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from papylonation_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from papylonation_cli.config import check_config_version, migrate_config
        ...
        try:
            import yaml
            from papylonation_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
    ...
    for subdir_name in expected_subdirs:
        subdir_path = papylonation_home / subdir_name
    ...
    soul_path = papylonation_home / "SOUL.md"
    if soul_path.exists():
    ...
    memories_dir = papylonation_home / "memories"
    ...
    state_db_path = papylonation_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from papylonation_state import _db_opens_cleanly, repair_state_db_schema
            ...
            from papylonation_state import is_malformed_db_error, repair_state_db_schema
    ...
    wal_path = papylonation_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    try:
        from papylonation_constants import is_container as _is_container
    ...
    try:
        from papylonation_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
