...
4. **Pip plugins**     – packages that expose the ``papylonation_agent.plugins``
   entry-point group.
...
from papylonation_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by papylonation_cli.kanban_db when a task
...
ENTRY_POINTS_GROUP = "papylonation_agent.plugins"

_NS_PARENT = "papylonation_plugins"
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        :func:`papylonation_cli.profiles.get_active_profile_name`, so it works in
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`papylonation_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_papylonation_dotenv()`` startup — after
        ...
        the first ``load_papylonation_dotenv()`` call, so a plugin-registered
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: papylonation_cli.main imports plugins indirectly
        from papylonation_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to papylonation_cli/. Two layouts are
        ...
        user_dir = get_papylonation_home() / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``papylonation_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``papylonation_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
