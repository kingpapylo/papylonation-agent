...
``~/.hermes/logs/`` (profile-aware via ``get_papylonation_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``papylonation_cli.web_server``,
    ``papylonation_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from papylonation_constants import get_config_path, get_papylonation_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_papylonation_session_injector", False):
    ...
    _session_record_factory._papylonation_session_injector = True  # type: ignore[attr-defined]
...
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "papylonation_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("papylonation_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "papylonation_cli.web_server",
        "papylonation_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    papylonation_home: Optional[Path] = None,
    ...
    *force* is ``True``.
    ...
    papylonation_home
        ...
        ``get_papylonation_home()`` (profile-aware).
    log_level
    ...
    home = papylonation_home or get_papylonation_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_papylonation_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._papylonation_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from papylonation_cli.config import is_managed
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._papylonation_queue = True  # type: ignore[attr-defined]
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_papylonation_queue", False):
