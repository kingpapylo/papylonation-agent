...
class MemoryManager:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``papylonation_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_papylonation_home()`` themselves.
        """
        if "papylonation_home" not in kwargs:
            from papylonation_constants import get_papylonation_home
            kwargs["papylonation_home"] = str(get_papylonation_home())
        for provider in self._providers:
            try:
