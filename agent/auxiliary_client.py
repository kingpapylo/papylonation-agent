...
# in lockstep with papylonation_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``papylonation_cli.__version__`` is
...
_AUTH_JSON_PATH = get_papylonation_home() / "auth.json"
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from papylonation_cli.auth import _nous_inference_env_override
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``papylonation_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_api_key(provider: dict) -> str:
    ...
    from papylonation_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from papylonation_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``papylonation_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from papylonation_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from papylonation_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
...
def _try_azure_foundry(
    *,
    ...
    :func:`papylonation_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from papylonation_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from papylonation_cli.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "copilot":
            from papylonation_cli.copilot_auth import (
                _jwt_cache,
                _token_fingerprint,
...
def _resolve_auto(
    ...
    if main_provider == "moa":
        try:
            from papylonation_cli.config import load_config
            from papylonation_cli.moa_config import resolve_moa_preset
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from papylonation_cli.runtime_provider import _get_named_custom_provider
...
def resolve_provider_client(
    ...
    try:
        ...
        logger.debug("papylonation_cli.auth not available for provider %s", provider)
        return None, None
...
def _resolve_task_provider_model(
    ...
    def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
        ...
        if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
            return False
        try:
            from papylonation_cli.providers import get_provider
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`papylonation_cli.plugins.PluginContext.register_auxiliary_task`) the
