...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`papylonation_cli.plugins.invoke_hook` and its aggregators.  Python
...
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``papylonation_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
from papylonation_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``papylonation_cli.config.load_config``
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`papylonation_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def allowlist_path() -> Path:
    ...
    return get_papylonation_home() / ALLOWLIST_FILENAME
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`papylonation_cli.plugins.invoke_hook`
