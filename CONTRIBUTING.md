...
- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(papylonation_home, config)` for setup-wizard integration
...
system `python3 -m papylonation_cli.main` can pick up unrelated system Python
packages.
...
├── papylonation_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── papylonation_cli/               # CLI command implementations
...
- **Session persistence**: All conversations are stored in SQLite (`papylonation_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.hermes/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
Add to `_BUILTIN_SKINS` dict in `papylonation_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `papylonation_cli/skin_engine.py` for the full schema and existing skins as examples.
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `papylonation_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `papylonation_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `papylonation_cli/gateway_windows.py::
