...
def set_papylonation_home_override(path: str | Path | None) -> Token:
...
def reset_papylonation_home_override(token: Token) -> None:
    """Restore the previous context-local Hermes home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_papylonation_home_override() -> str | None:
...
def _get_platform_default_papylonation_home() -> Path:
...
def get_papylonation_home() -> Path:
    ...
    template in ``papylonation_cli/gateway.py`` and the kanban dispatcher in
    ``papylonation_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_papylonation_home()
    ...
    return _get_platform_default_papylonation_home()


def get_default_papylonation_root() -> Path:
    ...
    """
    native_home = _get_platform_default_papylonation_home()
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_papylonation_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_papylonation_home() / "optional-mcps"
...
def get_papylonation_dir(new_subpath: str, old_name: str) -> Path:
...
def iter_papylonation_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    """
    root = home or get_papylonation_home()
    dirs = [root / "node"]
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    immediately throws ``MODULE_NOT_FOUND``. ``find_papylonation_node_executable``
...
def papylonation_managed_node_tree_present(home: Path | None = None) -> bool:
    ...
    for directory in iter_papylonation_node_dirs(home):
        for name in names:
...
def heal_papylonation_managed_node() -> bool:
    ...
    if _managed_node_heal_attempted:
        return False
    if not papylonation_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HERMES_HOME": str(get_papylonation_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_papylonation_node_executable(command: str) -> str | None:
    ...
    for directory in iter_papylonation_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_papylonation_managed_node():
        for directory in iter_papylonation_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_papylonation_node_executable(command)
    if managed:
        return managed
    if papylonation_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_papylonation_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Hermes-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_papylonation_node_dirs() if path.is_dir()]
...
def display_papylonation_home() -> str:
    ...
    :func:`get_papylonation_home` instead.
    """
    home = get_papylonation_home()
    try:
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    papylonation_home = get_papylonation_home_override() or (env or {}).get("HERMES_HOME") or os.getenv("HERMES_HOME")
    if not papylonation_home:
        return None
    profile_home = os.path.join(papylonation_home, "home")
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_papylonation_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, papylonation_logging.py, papylonation_time.py, etc.).
    """
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_papylonation_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._papylonation_ipv4_patched = True  # type: ignore[attr-defined]
